...
abstract class PoolArena<T> {
    ...
    private synchronized void allocateNormal(PooledByteBuf<T> buf, int reqCapacity, int normCapacity) {
        ...
        assert (handle > 0L);
        c2.initBuf(buf, handle, reqCapacity);
        this.qInit.add(c2);
    }
    ...
    public synchronized String toString() {
        ...
        for (i2 = 1; i2 < this.tinySubpagePools.length; ++i2) {
            head = this.tinySubpagePools[i2];
        ...
        for (i2 = 1; i2 < this.smallSubpagePools.length; ++i2) {
            head = this.smallSubpagePools[i2];
            if (head.next == head) continue;
            buf.append(StringUtil.NEWLINE);
            buf.append(i2);
            buf.append(": ");

...
implements ChannelOutboundHandler {
    ...
    private SSLEngineResult wrap(SSLEngine engine, ByteBuf in2, ByteBuf out) throws SSLException {
        SSLEngineResult result;
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws SSLException {
        ...
        while (totalLength < 18713 && (readableBytes = endOffset - offset) >= 5) {
            int packetLength = SslHandler.getEncryptedPacketLength(in2, offset);
            if (packetLength == -1) {
                nonSslRecord = true;
                break;
            }
        ...
        if (totalLength > 0) {
            ...
            ByteBuffer inNetBuf = in2.nioBuffer(startOffset, totalLength);
        ...
        block6: while (true) {
            ...
            switch (result.getStatus()) {
                case BUFFER_OVERFLOW: {
                    ...
                    switch (overflows++) {
                        case 0: {
                            out.ensureWritable(Math.min(max, in2.remaining()));
                            continue block6;
                        }
                    }

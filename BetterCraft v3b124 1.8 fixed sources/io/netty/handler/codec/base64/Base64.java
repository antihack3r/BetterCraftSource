...
public final class Base64 {
    ...
    public static ByteBuf encode(ByteBuf src, int off, int len, boolean breakLines, Base64Dialect dialect) {
        ...
        int lineLength = 0;
        while (d2 < len2) {
            Base64.encode3to4(src, d2 + off, 3, dest, e2, dialect);
        ...
        if (d2 < len) {
            Base64.encode3to4(src, d2 + off, len - d2, dest, e2, dialect);
            e2 += 4;
        }
    ...
    public static ByteBuf decode(ByteBuf src, int off, int len, Base64Dialect dialect) {
        ...
        for (int i2 = off; i2 < off + len; ++i2) {
            byte sbiCrop = (byte)(src.getByte(i2) & 0x7F);
            ...
            throw new IllegalArgumentException("bad Base64 input character at " + i2 + ": " + src.getUnsignedByte(i2) + " (decimal)");
        }

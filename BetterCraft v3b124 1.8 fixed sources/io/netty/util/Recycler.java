...
public abstract class Recycler<T> {
    ...
    private static final class WeakOrderQueue {
        ...
        boolean transfer(Stack<?> to2) {
            ...
            int count = end - start;
            if (((Stack)to2).size + count > ((Stack)to2).elements.length) {
                Stack.access$1202(to2, Arrays.copyOf(((Stack)to2).elements, (((Stack)to2).size + count) * 2));
            }
            DefaultHandle[] src = head.elements;
            DefaultHandle[] trg = ((Stack)to2).elements;
            int size = ((Stack)to2).size;
            while (start < end) {

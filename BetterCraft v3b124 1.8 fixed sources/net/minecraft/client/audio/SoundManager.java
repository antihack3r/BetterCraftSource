            ...
            if (this.sndSystem.getMasterVolume() <= 0.0f) {
                ...
                if (soundeventaccessorcomposite == null) {
                    ...
                    if (soundpoolentry == SoundHandler.missing_sound) {
                        ...
                        float f2 = p_sound.getVolume();
                        float f1 = 16.0f;
                        if (f2 > 1.0f) {
                            f1 *= f2;
                        }
                        SoundCategory soundcategory = soundeventaccessorcomposite.getSoundCategory();
                        float f22 = this.getNormalizedVolume(p_sound, soundpoolentry, soundcategory);
                        ...
                        if (f22 == 0.0f) {
                            ...
                            if (soundpoolentry.isStreamingSound()) {
                                this.sndSystem.newStreamingSource(false, s2, SoundManager.getURLForSoundResource(resourcelocation), resourcelocation.toString(), flag, p_sound.getXPosF(), p_sound.getYPosF(), p_sound.getZPosF(), p_sound.getAttenuationType().getTypeInt(), f1);
                            } else {
                                this.sndSystem.newSource(false, s2, SoundManager.getURLForSoundResource(resourcelocation), resourcelocation.toString(), flag, p_sound.getXPosF(), p_sound.getYPosF(), p_sound.getZPosF(), p_sound.getAttenuationType().getTypeInt(), f1);
                            }

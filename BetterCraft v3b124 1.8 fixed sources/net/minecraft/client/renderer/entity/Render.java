    ...
    private void renderShadow(Entity entityIn, double x2, double y2, double z2, float shadowAlpha, float partialTicks) {
        if (!Config.isShaders() || !Shaders.shouldSkipDefaultShadow) {
            ...
            for (BlockPos blockPos : BlockPos.getAllInBoxMutable(new BlockPos(i2, k2, i1), new BlockPos(j2, l2, j1))) {
                ...
                this.renderShadowBlock(block, x2, y2, z2, blockPos, shadowAlpha, f2, d2, d3, d4);
            }
            tessellator.draw();
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    protected void renderLivingLabel(T entityIn, String str, double x2, double y2, double z2, int maxDistance) {
        ...
        if (d0 <= (double)(maxDistance * maxDistance)) {
            ...
            if (entityIn instanceof EntityPlayer) {
                try {
                    String value = String.valueOf(String.valueOf(Math.round(((EntityPlayer)entityIn).getHealth()) / 2) + " \u00a7fhp \u00a78/ \u00a7r" + Math.round(((Entity)entityIn).getDistanceToEntity(Minecraft.getMinecraft().thePlayer)) + " m \u00a78/ \u00a7r" + Minecraft.getMinecraft().getNetHandler().getPlayerInfo(((Entity)entityIn).getUniqueID()).getResponseTime() + " ms");
                    fontrenderer.drawString(value, -fontrenderer.getStringWidth(value) / 2, i2 - 20, -1);
                    Minecraft.getMinecraft().getRenderItem().renderItemOverlays(fontrenderer, ((EntityPlayer)entityIn).getHeldItem(), -55, -55);
                    GlStateManager.pushMatrix();
                    GlStateManager.translate(-5.0f, 0.0f, -150.0f);
                    Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(((EntityPlayer)entityIn).getHeldItem(), -35, -40);
                    Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(((EntityPlayer)entityIn).getCurrentArmor(3), -20, -40);
                    Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(((EntityPlayer)entityIn).getCurrentArmor(2), -5, -40);
                    Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(((EntityPlayer)entityIn).getCurrentArmor(1), 10, -40);
                    Minecraft.getMinecraft().getRenderItem().renderItemAndEffectIntoGUI(((EntityPlayer)entityIn).getCurrentArmor(0), 25, -40);
                    GlStateManager.popMatrix();
                }

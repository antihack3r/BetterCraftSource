...
implements ARBDebugOutputCallback.Handler {
    ...
    @Override
    public void handleMessage(int source, int type, int id2, int severity, String message) {
        if (!(message.contains("glBindFramebuffer") || message.contains("Wide lines") || message.contains("shader recompiled"))) {
            Config.dbg("[LWJGL] source: " + this.getSource(source) + ", type: " + this.getType(type) + ", id: " + id2 + ", severity: " + this.getSeverity(severity) + ", message: " + message);

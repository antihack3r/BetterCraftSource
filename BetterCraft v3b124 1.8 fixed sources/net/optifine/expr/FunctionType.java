...
public enum FunctionType {
    ...
    public float evalFloat(IExpression[] args) {
        switch (this) {
            ...
            case IF: {
                int i2 = (args.length - 1) / 2;
                int k2 = 0;
                while (k2 < i2) {
                    int l2 = k2 * 2;
                    if (FunctionType.evalBool(args, l2)) {
                        return FunctionType.evalFloat(args, l2 + 1);
                    }
                    ++k2;
                }
                return FunctionType.evalFloat(args, i2 * 2);
            }
            case SMOOTH: {
                int j2 = (int)FunctionType.evalFloat(args, 0);
                ...
                float f7 = Smoother.getSmoothValue(j2, f4, f5, f6);
                return f7;
            }
        }
    ...
    private float getMin(IExpression[] exprs) {
        ...
        float f2 = FunctionType.evalFloat(exprs, 0);
        int i2 = 1;
        while (i2 < exprs.length) {
            float f1 = FunctionType.evalFloat(exprs, i2);
            if (f1 < f2) {
                f2 = f1;
            }
            ++i2;
    ...
    public boolean evalBool(IExpression[] args) {
        switch (this) {
            ...
            case BETWEEN: {
                float f2 = FunctionType.evalFloat(args, 0);
                return f2 >= FunctionType.evalFloat(args, 1) && f2 <= FunctionType.evalFloat(args, 2);
            }
            case EQUALS: {
            ...
            case IN: {
                ...
                while (i2 < args.length) {
                    float f4 = FunctionType.evalFloat(args, i2);
                    if (f3 == f4) {
                        return true;
                    }
                    ++i2;

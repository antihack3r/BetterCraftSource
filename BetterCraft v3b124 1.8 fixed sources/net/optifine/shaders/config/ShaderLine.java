...
public class ShaderLine {
    ...
    public Vector4f getValueVec4() {
        ...
        String s2 = this.value.trim();
        s2 = StrUtils.removePrefix(s2, "vec4");
        String[] astring = Config.tokenize(s2 = StrUtils.trim(s2, " ()"), ", ");
        if (astring.length != 4) {
            return null;
        }
        ...
        while (i2 < astring.length) {
            ...
            float f2 = Config.parseFloat(s1 = StrUtils.removeSuffix(s1, new String[]{"F", "f"}), Float.MAX_VALUE);

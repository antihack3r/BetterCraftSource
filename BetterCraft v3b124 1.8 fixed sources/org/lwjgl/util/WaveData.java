...
public class WaveData {
    ...
    public static WaveData create(AudioInputStream ais2) {
        AudioFormat audioformat = ais2.getFormat();
        int channels = 0;
        ...
        try {
            int available = ais2.available();
            if (available <= 0) {
                available = ais2.getFormat().getChannels() * (int)ais2.getFrameLength() * ais2.getFormat().getSampleSizeInBits() / 8;
            }
            byte[] buf = new byte[ais2.available()];
            int read = 0;
            for (int total = 0; (read = ais2.read(buf, total, buf.length - total)) != -1 && total < buf.length; total += read) {
            }

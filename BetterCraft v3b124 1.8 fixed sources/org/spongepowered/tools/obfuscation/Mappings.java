...
implements IMappingConsumer {
    ...
    @Override
    public void addFieldMapping(ObfuscationType type, MappingField from, MappingField to2) {
        ...
        mappings.add(new IMappingConsumer.MappingSet.Pair<MappingField>(from, to2));
    }

    @Override
    public void addMethodMapping(ObfuscationType type, MappingMethod from, MappingMethod to2) {
        IMappingConsumer.MappingSet<MappingMethod> mappings = this.methodMappings.get(type);
        if (mappings == null) {
            mappings = new IMappingConsumer.MappingSet();
            this.methodMappings.put(type, mappings);
        }
        mappings.add(new IMappingConsumer.MappingSet.Pair<MappingMethod>(from, to2));
    }
    ...
    implements IMappingConsumer {
        ...
        @Override
        public void addFieldMapping(ObfuscationType type, MappingField from, MappingField to2) {
            if (!this.checkForExistingMapping(type, from, to2, this.fields)) {
                this.mappings.addFieldMapping(type, from, to2);
            }
        }

        @Override
        public void addMethodMapping(ObfuscationType type, MappingMethod from, MappingMethod to2) {
            if (!this.checkForExistingMapping(type, from, to2, this.methods)) {
                this.mappings.addMethodMapping(type, from, to2);
            }
        }

        private <TMapping extends IMapping<TMapping>> boolean checkForExistingMapping(ObfuscationType type, TMapping from, TMapping to2, Map<ObfuscationType, Map<TMapping, TMapping>> mappings) throws MappingConflictException {
            IMapping existing;
            ...
            if ((existing = (IMapping)existingMappings.get(from)) != null) {
                if (existing.equals(to2)) {
                    return true;
                }
                throw new MappingConflictException(existing, to2);
            }
            existingMappings.put(from, to2);
            return false;
        }

        ...
        protocol.registerClientbound(ClientboundPackets1_8.ENTITY_POSITION, new PacketHandlers(){
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    ...
                    if (hologram != null) {
                        wrapper.cancel();
                        byte x2 = wrapper.get(Type.BYTE, 0);
                        byte y2 = wrapper.get(Type.BYTE, 1);
                        byte z2 = wrapper.get(Type.BYTE, 2);
                        hologram.handleOriginalMovementPacket((double)x2 / 32.0, (double)y2 / 32.0, (double)z2 / 32.0);
                    }
                });
            }
        });
            ...
            public void register() {
                ...
                this.handler(wrapper -> {
                    ...
                    if (type == EntityTypes1_10.EntityType.MINECART_ABSTRACT) {
                        int y2 = wrapper.get(Type.INT, 2);
                        wrapper.set(Type.INT, 2, y2 += 12);
                    }
                    if ((hologram = tracker.getVirtualHologramMap().get(entityId)) != null) {
                        wrapper.cancel();
                        int x2 = wrapper.get(Type.INT, 1);
                        int y3 = wrapper.get(Type.INT, 2);
                        int z2 = wrapper.get(Type.INT, 3);
                        byte yaw = wrapper.get(Type.BYTE, 0);
                        byte pitch = wrapper.get(Type.BYTE, 1);
                        hologram.updateReplacementPosition((double)x2 / 32.0, (double)y3 / 32.0, (double)z2 / 32.0);
                        hologram.setYawPitch((float)yaw * 360.0f / 256.0f, (float)pitch * 360.0f / 256.0f);
                    }
                });
            }

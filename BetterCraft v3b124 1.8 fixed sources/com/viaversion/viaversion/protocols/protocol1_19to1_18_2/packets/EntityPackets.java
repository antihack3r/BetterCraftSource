...
extends EntityRewriter<ClientboundPackets1_18, Protocol1_19To1_18_2> {
    ...
    private static void writeDimensionKey(PacketWrapper wrapper, DimensionRegistryStorage registryStorage) throws Exception {
        ...
        if (dimensionKey == null) {
            ...
            dimensionKey = (String)((Map.Entry)registryStorage.dimensions().entrySet().stream().map(it2 -> new Pair((Map.Entry)it2, Maps.difference(currentDimension.getValue(), ((CompoundTag)it2.getKey()).getValue()).entriesInCommon())).filter(it2 -> ((Map)it2.value()).containsKey("min_y") && ((Map)it2.value()).containsKey("height")).max(Comparator.comparingInt(it2 -> ((Map)it2.value()).size())).orElseThrow(() -> new IllegalArgumentException("Dimension not found in registry data from join packet: " + currentDimension)).key()).getValue();
        }
        wrapper.write(Type.STRING, dimensionKey);
    }

    private static int to3dId(int id2) {
        switch (id2) {
            case -1: {
                return 1;
            }
        ...
        throw new IllegalArgumentException("Unknown 2d id: " + id2);
    }

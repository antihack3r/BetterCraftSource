...
extends EntityRewriter<ClientboundPackets1_13, Protocol1_14To1_13_2> {
    ...
    protected void handleMetadata(int entityId, EntityType type, Metadata metadata, List<Metadata> metadatas, UserConnection connection) throws Exception {
        ...
        if (metadata.id() == 8 && type.isOrHasParent(EntityTypes1_14.LIVINGENTITY) && Float.isNaN(v2 = ((Number)metadata.getValue()).floatValue()) && Via.getConfig().is1_14HealthNaNFix()) {
        ...
        } else if (type.is((EntityType)EntityTypes1_14.HORSE)) {
            if (metadata.id() == 18) {
                metadatas.remove(metadata);
        ...
        } else if (type.is((EntityType)EntityTypes1_14.VILLAGER)) {
            if (metadata.id() == 15) {
        ...
        } else if (type.is((EntityType)EntityTypes1_14.ZOMBIE_VILLAGER)) {
            if (metadata.id() == 18) {
                metadata.setTypeAndValue(Types1_14.META_TYPES.villagerDatatType, new VillagerData(2, MetadataRewriter1_14To1_13_2.getNewProfessionId((Integer)metadata.getValue()), 0));
            }
        ...
        } else if (type.is((EntityType)EntityTypes1_14.FIREWORK_ROCKET)) {
            if (metadata.id() == 8) {
        ...
        if ((type.is((EntityType)EntityTypes1_14.WITCH) || type.is((EntityType)EntityTypes1_14.RAVAGER) || type.isOrHasParent(EntityTypes1_14.ABSTRACT_ILLAGER_BASE)) && metadata.id() >= 14) {
            metadata.setId(metadata.id() + 1);
        }
    }

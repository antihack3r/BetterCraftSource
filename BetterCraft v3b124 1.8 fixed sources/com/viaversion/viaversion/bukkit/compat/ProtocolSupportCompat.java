        ...
        try {
            ...
            Bukkit.getPluginManager().registerEvent(connectionOpenEvent, new Listener(){}, EventPriority.HIGH, (listener, event) -> {
                try {
                    ...
                    Via.getPlatform().getLogger().log(Level.WARNING, "Error when handling ProtocolSupport event", e2);
                }
            }, (Plugin)plugin);
        }
        catch (ClassNotFoundException e2) {
            Via.getPlatform().getLogger().log(Level.WARNING, "Unable to register ProtocolSupport listener", e2);
        }
    }

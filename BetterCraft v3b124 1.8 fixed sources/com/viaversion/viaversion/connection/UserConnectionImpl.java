...
implements UserConnection {
    ...
    private void sendRawPacket(ByteBuf packet, boolean currentThread) {
        Runnable act2 = this.clientSide ? () -> this.getChannel().pipeline().context(Via.getManager().getInjector().getDecoderName()).fireChannelRead(packet) : () -> this.channel.pipeline().context(Via.getManager().getInjector().getEncoderName()).writeAndFlush(packet);
        if (currentThread) {
            act2.run();
        } else {
            try {
                this.channel.eventLoop().submit(act2);
            }
            catch (Throwable e2) {
                packet.release();
                e2.printStackTrace();
    ...
    private void sendRawPacketToServerClientSide(ByteBuf packet, boolean currentThread) {
        Runnable act2 = () -> this.getChannel().pipeline().context(Via.getManager().getInjector().getEncoderName()).writeAndFlush(packet);
        if (currentThread) {
            act2.run();
        } else {
            try {
                this.getChannel().eventLoop().submit(act2);
            }
            catch (Throwable e2) {
                e2.printStackTrace();
                packet.release();
    ...
    private void transform(ByteBuf buf, Direction direction, Function<Throwable, Exception> cancelSupplier) throws Exception {
        ...
        int id2 = Type.VAR_INT.readPrimitive(buf);
        if (id2 == 1000) {
        ...
        PacketWrapperImpl wrapper = new PacketWrapperImpl(id2, buf, (UserConnection)this);

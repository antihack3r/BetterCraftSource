...
implements ConnectionManager {
    ...
    public void onLoginSuccess(UserConnection connection) {
        ...
        if (this.isFrontEnd(connection) && (previous = this.clients.put(id2 = connection.getProtocolInfo().getUuid(), connection)) != null && previous != connection) {
            Via.getPlatform().getLogger().warning("Duplicate UUID on frontend connection! (" + id2 + ")");
        }
        if (channel != null) {
            if (!channel.isOpen()) {
    ...
    public void onDisconnect(UserConnection connection) {
        ...
        if (this.isFrontEnd(connection)) {
            UUID id2 = connection.getProtocolInfo().getUuid();
            this.clients.remove(id2);
        }

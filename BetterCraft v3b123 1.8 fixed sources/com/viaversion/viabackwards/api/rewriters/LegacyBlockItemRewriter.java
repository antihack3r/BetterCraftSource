    ...
    protected void handleChunk(final Chunk chunk) {
        ...
        for (final CompoundTag tag : chunk.getBlockEntities()) {
            ...
            if ((xTag = tag.get("x")) != null && (yTag = tag.get("y")) != null) {
                ...
                final int block = section.palette(PaletteType.BLOCKS).idAt(pos.getX(), pos.getY() & 0xF, pos.getZ());
        ...
        for (int i = 0; i < chunk.getSections().length; ++i) {
            ...
            if (section2 != null) {
                ...
                for (int j = 0; j < palette.size(); ++j) {
                    final int block2 = palette.idByIndex(j);
                    ...
                    if (!hasBlockEntityHandler) {
                        final MappedLegacyBlockItem settings = this.replacementData.get(btype2);
                        if (settings != null && settings.hasBlockEntityHandler()) {
                ...
                if (hasBlockEntityHandler) {
                    for (int x = 0; x < 16; ++x) {
                        for (int y = 0; y < 16; ++y) {
                            for (int z = 0; z < 16; ++z) {
                                final int block = palette.idAt(x, y, z);
                                final int btype = block >> 4;
                                final int meta2 = block & 0xF;
                                final MappedLegacyBlockItem settings2 = this.replacementData.get(btype);
                                if (settings2 != null) {
                                    if (settings2.hasBlockEntityHandler()) {
                                        ...
                                        if (!tags.containsKey(pos2)) {
                                            ...
                                            settings2.getBlockEntityHandler().handleOrNewCompoundTag(block, tag2);

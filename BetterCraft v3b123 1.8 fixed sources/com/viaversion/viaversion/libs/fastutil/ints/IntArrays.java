            ...
            if (stable) {
                ...
                while (j <= lastUsed) {
                    if (level < 3 && count[j] > 1) {
                        if (count[j] < 1024) {
                            quickSortIndirect(perm, a, p, p + count[j]);
                            if (stable) {
                                stabilize(perm, a, p, p + count[j]);
                            }
                        }
                        else {
                ...
                for (int end = first + length - count[lastUsed], k = first, c = -1; k <= end; k += count[c], count[c] = 0) {
                    ...
                    if (level < 3 && count[c] > 1) {
                        if (count[c] < 1024) {
                            quickSortIndirect(perm, a, k, k + count[c]);
                            if (stable) {
                                stabilize(perm, a, k, k + count[c]);
                            }
                        }
                        else {
                            offsetStack[stackPos] = k;

    ...
    public static final class Readers
    {
        ...
        static {
            ...
            VIBRATION = ((buf, particle) -> {
                ...
                final String resourceLocation2 = Key.stripMinecraftNamespace(resourceLocation);
                if (resourceLocation2.equals("block")) {
                    particle.add(Type.POSITION1_14, Type.POSITION1_14.read(buf));
                }
                else if (resourceLocation2.equals("entity")) {
                    particle.add(Type.VAR_INT, Type.VAR_INT.readPrimitive(buf));
                }
                else {
                    Via.getPlatform().getLogger().warning("Unknown vibration path position source type: " + resourceLocation2);
                }
                particle.add(Type.VAR_INT, Type.VAR_INT.readPrimitive(buf));
                return;
            });

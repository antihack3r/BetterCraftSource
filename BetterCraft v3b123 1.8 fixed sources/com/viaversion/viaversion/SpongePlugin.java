...
public class SpongePlugin implements ViaPlatform<Player>
{
    ...
    public PlatformTask runSync(final Runnable runnable, final long delay) {
        final Task task = Task.builder().plugin(this.container).execute(runnable).delay(Ticks.of(delay)).build();
        return new SpongeViaTask(this.game.server().scheduler().submit(task));
    }
    
    @Override
    public PlatformTask runRepeatingSync(final Runnable runnable, final long period) {
        final Task task = Task.builder().plugin(this.container).execute(runnable).interval(Ticks.of(period)).build();
        return new SpongeViaTask(this.game.server().scheduler().submit(task));
    }

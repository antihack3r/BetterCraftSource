...
public final class WorldPackets
{
    public static void register(final Protocol1_18To1_17_1 protocol) {
        ((AbstractProtocol<ClientboundPackets1_17_1, CM, SM, SU>)protocol).registerClientbound(ClientboundPackets1_17_1.BLOCK_ENTITY_DATA, new PacketHandlers() {
        ...
        ((AbstractProtocol<ClientboundPackets1_17_1, CM, SM, SU>)protocol).registerClientbound(ClientboundPackets1_17_1.UPDATE_LIGHT, wrapper -> {
        ...
        ((AbstractProtocol<ClientboundPackets1_17_1, CM, SM, SU>)protocol).registerClientbound(ClientboundPackets1_17_1.CHUNK_DATA, wrapper -> {
            ...
            for (int k = 0; k < sections.length; ++k) {
                ChunkSection section = sections[k];
                if (section == null) {
                    section = new ChunkSectionImpl();
                    (sections[k] = section).setNonAirBlocksCount(0);
            ...
            final ChunkLightStorage lightStorage2 = wrapper.user().get(ChunkLightStorage.class);
            final boolean alreadyLoaded = !lightStorage2.addLoadedChunk(chunk.getX(), chunk.getZ());
            final ChunkLightStorage.ChunkLight light = Via.getConfig().cache1_17Light() ? lightStorage2.getLight(chunk.getX(), chunk.getZ()) : lightStorage2.removeLight(chunk.getX(), chunk.getZ());
            if (light == null) {
                ...
                final byte[][] array2;
                int n = 0;
                for (int length2 = array2.length; n < length2; ++n) {
                    final byte[] blockLight2 = array2[n];
                    wrapper.write(Type.BYTE_ARRAY_PRIMITIVE, blockLight2);
                }
            }
            return;
        });
        ((AbstractProtocol<ClientboundPackets1_17_1, CM, SM, SU>)protocol).registerClientbound(ClientboundPackets1_17_1.UNLOAD_CHUNK, wrapper -> {
            final int chunkX2 = wrapper.passthrough((Type<Integer>)Type.INT);
            final int chunkZ2 = wrapper.passthrough((Type<Integer>)Type.INT);
            wrapper.user().get(ChunkLightStorage.class).clear(chunkX2, chunkZ2);
        });
    }

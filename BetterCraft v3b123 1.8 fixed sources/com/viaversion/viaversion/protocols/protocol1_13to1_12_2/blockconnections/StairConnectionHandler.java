...
{
    private static final Int2ObjectMap<StairData> STAIR_DATA_MAP;
    private static final Map<Short, Integer> CONNECTED_BLOCKS;
    ...
    public int connect(final UserConnection user, final Position position, final int blockState) {
        final StairData stairData = StairConnectionHandler.STAIR_DATA_MAP.get(blockState);
        if (stairData == null) {
            return blockState;
        }
        short s = 0;
        ...
        s |= (short)(stairData.getType() << 4);
        s |= (short)(stairData.getFacing().ordinal() << 9);
        final Integer newBlockState = StairConnectionHandler.CONNECTED_BLOCKS.get(s);
    ...
    private int getShape(final UserConnection user, final Position position, final StairData stair) {
        ...
        StairData relativeStair = StairConnectionHandler.STAIR_DATA_MAP.get(this.getBlockData(user, position.getRelative(facing)));
        ...
        relativeStair = StairConnectionHandler.STAIR_DATA_MAP.get(this.getBlockData(user, position.getRelative(facing.opposite())));
        if (relativeStair != null && relativeStair.isBottom() == stair.isBottom()) {
            final BlockFace facing2 = relativeStair.getFacing();
    ...
    private boolean checkOpposite(final UserConnection user, final StairData stair, final Position position, final BlockFace face) {
        final StairData relativeStair = StairConnectionHandler.STAIR_DATA_MAP.get(this.getBlockData(user, position.getRelative(face)));
    ...
    static {
        STAIR_DATA_MAP = new Int2ObjectOpenHashMap<StairData>();
        CONNECTED_BLOCKS = new HashMap<Short, Integer>();
    }

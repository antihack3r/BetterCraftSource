            ...
            new Thread("MCO Availability Checker #1") {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < 3; ++i) {
                        ...
                        catch (final RetryCallException e3) {
                            retry = true;
                        }
                        catch (final RealmsServiceException e4) {
                            RealmsMainScreen.LOGGER.error("Couldn't connect to Realms: " + e4.toString());
                        }
                        catch (final IOException e5) {
                            RealmsMainScreen.LOGGER.error("Couldn't parse response connecting to Realms: " + e5.getMessage());
                        }
                        if (!retry) {
                            break;
                        }

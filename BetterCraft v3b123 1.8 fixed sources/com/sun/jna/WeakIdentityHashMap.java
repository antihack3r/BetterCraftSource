...
public class WeakIdentityHashMap implements Map
{
    ...
    public Set entrySet() {
        ...
        final Iterator i = this.backingStore.entrySet().iterator();
        while (i.hasNext()) {
            final Entry ref = i.next();
    ...
    public Set keySet() {
        this.reap();
        final Set ret = new HashSet();
        final Iterator i = this.backingStore.keySet().iterator();
        while (i.hasNext()) {
            final IdentityWeakReference ref = i.next();
            ret.add(ref.get());
        }
        return Collections.unmodifiableSet((Set<?>)ret);
    }

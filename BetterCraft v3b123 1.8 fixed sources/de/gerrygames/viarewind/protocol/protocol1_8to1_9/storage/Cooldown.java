...
public class Cooldown extends StoredObject implements Tickable
{
    ...
    public void setAttackSpeed(final double base, final ArrayList<Pair<Byte, Double>> modifiers) {
        ...
        for (int j = 0; j < modifiers.size(); ++j) {
            if ((byte)modifiers.get(j).key() == 0) {
                this.attackSpeed += (double)modifiers.get(j).value();
                modifiers.remove(j--);
            }
        }
        for (int j = 0; j < modifiers.size(); ++j) {
            if ((byte)modifiers.get(j).key() == 1) {
                this.attackSpeed += base * (double)modifiers.get(j).value();
                modifiers.remove(j--);
            }
        }
        for (int j = 0; j < modifiers.size(); ++j) {
            if ((byte)modifiers.get(j).key() == 2) {
                this.attackSpeed *= 1.0 + (double)modifiers.get(j).value();
                modifiers.remove(j--);
            }
        }
    }

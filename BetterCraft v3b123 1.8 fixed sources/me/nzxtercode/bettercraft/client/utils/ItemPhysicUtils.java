...
public class ItemPhysicUtils
{
    ...
    public static void doRender(final Entity par1Entity, final double x, final double y, final double z, final float par8, final float par9) {
        ItemPhysicUtils.rotation = (System.nanoTime() - ItemPhysicUtils.tick) / 3000000.0 * 0.1;
        ...
        if (itemstack.getItem() != null) {
            ...
            if (entityitem != null && !Double.isNaN(entityitem.posX) && !Double.isNaN(entityitem.posY) && !Double.isNaN(entityitem.posZ) && Minecraft.getMinecraft().theWorld != null) {
                if (entityitem.onGround) {
                    ...
                    blockpos2.add(0, 1, 0);
                    final Material material = Minecraft.getMinecraft().theWorld.getBlockState(blockpos2).getBlock().getMaterial();
                    final Material material2 = Minecraft.getMinecraft().theWorld.getBlockState(blockpos).getBlock().getMaterial();
                    ...
                    if (flag2 | material == Material.water | material2 == Material.water | flag3) {
                        ...
                        entityitem.rotationPitch += (float)(ItemPhysicUtils.rotation * 3.0);
                    }
                }
            }

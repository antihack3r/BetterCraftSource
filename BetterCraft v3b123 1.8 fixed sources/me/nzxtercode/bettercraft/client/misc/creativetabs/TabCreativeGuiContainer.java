...
public class TabCreativeGuiContainer extends InventoryEffectRenderer
{
    ...
    static {
        ...
        TabCreativeGuiContainer.field_147060_v = new InventoryBasic("tmp", true, 45);
        TabCreativeGuiContainer.selectedTabIndex = TabsCreative.tabMisc.getTabIndex();
        TabCreativeGuiContainer.tabPage = 0;
    }
    
    public TabCreativeGuiContainer(final EntityPlayer p_i1088_1_) {
    ...
    public void handleMouseClick(final Slot slotIn, final int slotId, final int clickedButton, int clickType) {
        this.field_147057_D = true;
        final boolean flag = clickType == 1;
        clickType = ((slotId == -999 && clickType == 0) ? 4 : clickType);
        if (slotIn == null && TabCreativeGuiContainer.selectedTabIndex != TabsCreative.tabInventory.getTabIndex() && clickType != 5) {
        ...
        else if (TabCreativeGuiContainer.selectedTabIndex == TabsCreative.tabInventory.getTabIndex()) {
        ...
        else if (clickType != 5 && slotIn.inventory == TabCreativeGuiContainer.field_147060_v) {
    ...
    public void initGui() {
        if (this.mc.playerController.isInCreativeMode()) {
            ...
            final int tabCount = TabsCreative.creativeTabArray.length;
            if (tabCount > 12) {
                this.maxPages = (tabCount - 12) / 10 + 1;
            }
            this.buttonList.add(new GuiButton(45, TabCreativeGuiContainer.width / 2 - 7, TabCreativeGuiContainer.height / 2 - 117, 20, 20, "<"));
            ...
            final int i = TabCreativeGuiContainer.selectedTabIndex;
            TabCreativeGuiContainer.selectedTabIndex = -1;
            this.setCurrentCreativeTab(TabsCreative.creativeTabArray[i]);
    ...
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        if (TabCreativeGuiContainer.selectedTabIndex != TabsCreative.tabAllSearch.getTabIndex()) {
            if (GameSettings.isKeyDown(this.mc.gameSettings.keyBindChat)) {
                this.setCurrentCreativeTab(TabsCreative.tabAllSearch);
            }
            else {
    ...
    private void updateCreativeSearch() {
        ...
        for (final Item item : Item.itemRegistry) {
            if (item != null && item.getCreativeTab() != null) {
                item.getSubItems(item, (TabsCreative)null, guicontainercreative$containercreative.itemList);
            }
        }
    ...
    protected void drawGuiContainerForegroundLayer(final int mouseX, final int mouseY) {
        final TabsCreative creativetabs = TabsCreative.creativeTabArray[TabCreativeGuiContainer.selectedTabIndex];
    ...
    protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) throws IOException {
        if (mouseButton == 0) {
            ...
            TabsCreative[] creativeTabArray;
            for (int length = (creativeTabArray = TabsCreative.creativeTabArray).length, k = 0; k < length; ++k) {
                final TabsCreative creativetabs = creativeTabArray[k];
    ...
    private boolean needsScrollBars() {
        return TabCreativeGuiContainer.selectedTabIndex != TabsCreative.tabInventory.getTabIndex() && TabsCreative.creativeTabArray[TabCreativeGuiContainer.selectedTabIndex].shouldHidePlayerInventory() && ((ContainerCreative)this.inventorySlots).func_148328_e();
    }
    
    private void setCurrentCreativeTab(final TabsCreative p_147050_1_) {
        final int i = TabCreativeGuiContainer.selectedTabIndex;
        TabCreativeGuiContainer.selectedTabIndex = p_147050_1_.getTabIndex();
        final ContainerCreative guicontainercreative$containercreative = (ContainerCreative)this.inventorySlots;
        this.dragSplittingSlots.clear();
        guicontainercreative$containercreative.itemList.clear();
        p_147050_1_.displayAllReleventItems(guicontainercreative$containercreative.itemList);
        if (p_147050_1_ == TabsCreative.tabInventory) {
            ...
            this.field_147064_C = new Slot(TabCreativeGuiContainer.field_147060_v, 0, 173, 112);
            guicontainercreative$containercreative.inventorySlots.add(this.field_147064_C);
        }
        else if (i == TabsCreative.tabInventory.getTabIndex()) {
            guicontainercreative$containercreative.inventorySlots = this.field_147063_B;
            this.field_147063_B = null;
        }
        if (this.searchField != null) {
            if (p_147050_1_ == TabsCreative.tabAllSearch) {
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        int start = TabCreativeGuiContainer.tabPage * 10;
        final int end = Math.min(TabsCreative.creativeTabArray.length, (TabCreativeGuiContainer.tabPage + 1) * 10 + 2);
        if (TabCreativeGuiContainer.tabPage != 0) {
            start += 2;
        }
        boolean rendered = false;
        for (final TabsCreative creativetabs : Arrays.copyOfRange(TabsCreative.creativeTabArray, start, end)) {
            if (creativetabs != null && this.renderCreativeInventoryHoveringText(creativetabs, mouseX, mouseY)) {
                rendered = true;
                break;
            }
        }
        if (!rendered && this.renderCreativeInventoryHoveringText(TabsCreative.tabAllSearch, mouseX, mouseY)) {
            this.renderCreativeInventoryHoveringText(TabsCreative.tabInventory, mouseX, mouseY);
        }
        if (this.field_147064_C != null && TabCreativeGuiContainer.selectedTabIndex == TabsCreative.tabInventory.getTabIndex() && this.isPointInRegion(this.field_147064_C.xDisplayPosition, this.field_147064_C.yDisplayPosition, 16, 16, mouseX, mouseY)) {
            this.drawCreativeTabHoveringText(I18n.format("inventory.binSlot", new Object[0]), mouseX, mouseY);
        }
        if (this.maxPages != 0) {
            final String page = String.format("%d / %d", TabCreativeGuiContainer.tabPage + 1, this.maxPages + 1);
    ...
    protected void renderToolTip(final ItemStack stack, final int x, final int y) {
        if (TabCreativeGuiContainer.selectedTabIndex == TabsCreative.tabAllSearch.getTabIndex()) {
            ...
            TabsCreative creativetabs = stack.getItem().getTabCreative();
            if (creativetabs == null && stack.getItem() == Items.enchanted_book) {
                ...
                if (map.size() == 1) {
                    ...
                    TabsCreative[] creativeTabArray;
                    for (int length = (creativeTabArray = TabsCreative.creativeTabArray).length, j = 0; j < length; ++j) {
                        final TabsCreative creativetabs2 = creativeTabArray[j];
    ...
    protected void drawGuiContainerBackgroundLayer(final float partialTicks, final int mouseX, final int mouseY) {
        ...
        for (final TabsCreative creativetabs2 : Arrays.copyOfRange(TabsCreative.creativeTabArray, start, end)) {
            this.mc.getTextureManager().bindTexture(TabCreativeGuiContainer.creativeInventoryTabs);
            if (creativetabs2 != null && creativetabs2.getTabIndex() != TabCreativeGuiContainer.selectedTabIndex) {
                this.func_147051_a(creativetabs2);
            }
        }
        if (TabCreativeGuiContainer.tabPage != 0) {
            if (creativetabs != TabsCreative.tabAllSearch) {
                this.mc.getTextureManager().bindTexture(TabCreativeGuiContainer.creativeInventoryTabs);
                this.func_147051_a(TabsCreative.tabAllSearch);
            }
            if (creativetabs != TabsCreative.tabInventory) {
                this.mc.getTextureManager().bindTexture(TabCreativeGuiContainer.creativeInventoryTabs);
                this.func_147051_a(TabsCreative.tabInventory);
            }
        }
        ...
        if ((creativetabs == null || creativetabs.getTabPage() != TabCreativeGuiContainer.tabPage) && creativetabs != TabsCreative.tabAllSearch && creativetabs != TabsCreative.tabInventory) {
            return;
        }
    ...
    protected boolean func_147049_a(final TabsCreative p_147049_1_, final int p_147049_2_, final int p_147049_3_) {
        if (p_147049_1_.getTabPage() != TabCreativeGuiContainer.tabPage && p_147049_1_ != TabsCreative.tabAllSearch && p_147049_1_ != TabsCreative.tabInventory) {
            return false;
        }
    ...
    protected boolean renderCreativeInventoryHoveringText(final TabsCreative p_147052_1_, final int p_147052_2_, final int p_147052_3_) {
    ...
    protected void func_147051_a(final TabsCreative p_147051_1_) {
        final boolean flag = p_147051_1_.getTabIndex() == TabCreativeGuiContainer.selectedTabIndex;
    ...
    protected void actionPerformed(final GuiButton button) throws IOException {
        ...
        if (button.id == 101) {
            TabCreativeGuiContainer.tabPage = 1;
        }
        if (button.id == 102) {
            TabCreativeGuiContainer.tabPage = 0;
        }
    ...
    public int getSelectedTabIndex() {
        return TabCreativeGuiContainer.selectedTabIndex;
    }
    ...
    static class ContainerCreative extends Container
    {
        ...
        public ContainerCreative(final EntityPlayer p_i1086_1_) {
            ...
            for (int i = 0; i < 5; ++i) {
                for (int j = 0; j < 9; ++j) {
                    this.addSlotToContainer(new Slot(TabCreativeGuiContainer.field_147060_v, i * 9 + j, 9 + j * 18, 18 + i * 18));
                }
            }
            for (int k = 0; k < 9; ++k) {
            ...
            for (int k = 0; k < 5; ++k) {
                for (int l = 0; l < 9; ++l) {
                    ...
                    if (i2 >= 0 && i2 < this.itemList.size()) {
                        TabCreativeGuiContainer.field_147060_v.setInventorySlotContents(l + k * 9, this.itemList.get(i2));
                    }
                    else {
                        TabCreativeGuiContainer.field_147060_v.setInventorySlotContents(l + k * 9, null);
                    }
                }
            }
        }

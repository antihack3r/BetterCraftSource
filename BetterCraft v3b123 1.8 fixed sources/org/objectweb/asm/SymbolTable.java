...
final class SymbolTable
{
    ...
    private Entry put(final Entry entry) {
        if (this.entryCount > this.entries.length * 3 / 4) {
            ...
            final Entry[] arrayOfEntry = new Entry[newCapacity];
    ...
    int addMergedType(final int typeTableIndex1, final int typeTableIndex2) {
        final long data = typeTableIndex1 | typeTableIndex2 << 32;
    ...
    private int addTypeInternal(final Entry entry) {
        ...
        if (this.typeCount == this.typeTable.length) {
            final Entry[] arrayOfEntry = new Entry[2 * this.typeTable.length];

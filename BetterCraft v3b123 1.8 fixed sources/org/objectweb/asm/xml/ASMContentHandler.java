...
public class ASMContentHandler extends DefaultHandler implements Opcodes
{
    ...
    private static void addOpcode(final String s, final int n, final int n2) {
        ASMContentHandler.OPCODES.put(s, new ASMContentHandler$Opcode(n, n2));
    }
    
    public ASMContentHandler(final ClassVisitor cv) {
        this.stack = new ArrayList();
        this.match = "";
        (this.RULES = new ASMContentHandler$RuleSet()).add("class", new ASMContentHandler$ClassRule(this));
    ...
    public final void startElement(final String s, final String s2, final String s3, final Attributes attributes) throws SAXException {
        final String s4 = (s2 == null || s2.length() == 0) ? s3 : s2;
        ...
        final ASMContentHandler$Rule asmContentHandler$Rule = (ASMContentHandler$Rule)this.RULES.match(this.match);
        if (asmContentHandler$Rule != null) {
            asmContentHandler$Rule.begin(s4, attributes);
        }
    }
    ...
    public final void endElement(final String s, final String s2, final String s3) throws SAXException {
        ...
        final ASMContentHandler$Rule asmContentHandler$Rule = (ASMContentHandler$Rule)this.RULES.match(this.match);
        if (asmContentHandler$Rule != null) {
            asmContentHandler$Rule.end(s4);
        }
        final int lastIndex = this.match.lastIndexOf(47);
        if (lastIndex >= 0) {
            this.match = this.match.substring(0, lastIndex);
        }
        else {
            this.match = "";
        }

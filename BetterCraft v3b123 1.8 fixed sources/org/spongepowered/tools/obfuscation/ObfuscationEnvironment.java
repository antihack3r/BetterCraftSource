...
public abstract class ObfuscationEnvironment implements IObfuscationEnvironment
{
    ...
    @Override
    public MappingMethod getObfMethod(final ITargetSelectorRemappable method) {
        final MappingMethod obfd = this.getObfMethod(method.asMethodMapping());
        if (obfd != null || !method.isFullyQualified()) {
            return obfd;
        }
        final TypeHandle type = this.ap.getTypeProvider().getTypeHandle(method.getOwner());
    ...
    public MappingMethod getObfMethod(final MappingMethod method, final boolean lazyRemap) {
        ...
        for (MappingMethod md = method; md != null && mapping == null; mapping = this.mappingProvider.getMethodMapping(md), md = md.getSuper(), superRef = true) {}
        if (mapping == null) {
            if (lazyRemap) {
                return null;
            }
            ...
            final String obfOwner = this.getObfClass(method.getOwner());
    ...
    @Override
    public ITargetSelectorRemappable remapDescriptor(final ITargetSelectorRemappable method) {
        ...
        if (desc != null) {
            final String newDesc = ObfuscationUtil.mapDescriptor(method.getDesc(), this.remapper);
    ...
    @Override
    public MappingField getObfField(final ITargetSelectorRemappable field) {

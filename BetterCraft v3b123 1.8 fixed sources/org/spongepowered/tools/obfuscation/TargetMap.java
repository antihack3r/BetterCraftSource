...
public final class TargetMap extends HashMap<TypeReference, Set<TypeReference>>
{
    ...
    void registerTargets(final AnnotatedMixin mixin) {
        this.registerTargets(mixin.getTargets(), mixin.getHandle());
    }
    
    void registerTargets(final List<TypeHandle> targets, final TypeHandle mixin) {
        for (final TypeHandle target : targets) {
            this.addMixin(target, mixin);
        }
    }
    
    void addMixin(final TypeHandle target, final TypeHandle mixin) {
        this.addMixin(target.getReference(), mixin.getReference());
    }
    
    void addMixin(final String target, final String mixin) {
        this.addMixin(new TypeReference(target), new TypeReference(mixin));
    }
    
    void addMixin(final TypeReference target, final TypeReference mixin) {
        final Set<TypeReference> mixins = this.getMixinsFor(target);
        mixins.add(mixin);
    }
    
    Collection<TypeReference> getMixinsTargeting(final TypeElement target) {
        return this.getMixinsTargeting(new TypeHandle(target));
    }
    
    Collection<TypeReference> getMixinsTargeting(final TypeHandle target) {
        return this.getMixinsTargeting(target.getReference());
    }
    
    Collection<TypeReference> getMixinsTargeting(final TypeReference target) {

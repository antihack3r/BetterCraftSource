...
public class ReferenceManager implements IReferenceManager
{
    ...
    private PrintWriter newWriter(final String fileName, final String description) throws IOException {
        ...
        this.ap.printMessage(IMessagerEx.MessageType.INFO, "Writing " + description + " to " + new File(outResource.toUri()).getAbsolutePath());
    ...
    @Override
    public void addMethodMapping(final String className, final String reference, final ITargetSelectorRemappable context, final ObfuscationData<MappingMethod> obfMethodData) {
        for (final ObfuscationEnvironment env : this.environments) {
            final MappingMethod obfMethod = obfMethodData.get(env.getType());
            if (obfMethod != null) {
                final ITargetSelectorRemappable remappedReference = context.remapUsing(obfMethod, true);
                this.addMapping(env.getType(), className, reference, remappedReference.toString());
            }
        }
    }
    ...
    @Override
    public void addFieldMapping(final String className, final String reference, final ITargetSelectorRemappable context, final ObfuscationData<MappingField> obfFieldData) {
        for (final ObfuscationEnvironment env : this.environments) {
            final MappingField obfField = obfFieldData.get(env.getType());
            if (obfField != null) {
                final MemberInfo remappedReference = MemberInfo.fromMapping(obfField.transform(env.remapDescriptor(context.getDesc())));
                this.addMapping(env.getType(), className, reference, remappedReference.toString());
            }
        }
    }

...
public class Yaml
{
    ...
    public <T> T loadAs(final Reader io, final Class<? super T> type) {
        return (T)this.loadFromReader(new StreamReader(io), type);
    }
    
    public <T> T loadAs(final String yaml, final Class<? super T> type) {
        return (T)this.loadFromReader(new StreamReader(yaml), type);
    }
    
    public <T> T loadAs(final InputStream input, final Class<? super T> type) {
    ...
    private Object loadFromReader(final StreamReader sreader, final Class<?> type) {
        final Composer composer = new Composer(new ParserImpl(sreader, this.loadingConfig), this.resolver, this.loadingConfig);

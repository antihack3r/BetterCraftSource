...
public class PropertySubstitute extends Property
{
    ...
    public void set(final Object object, final Object value) throws Exception {
        if (this.write != null) {
            ...
            PropertySubstitute.log.warn("No setter/delegate for '" + this.getName() + "' on object " + object);
        }
    }
    ...
    public void setTargetType(final Class<?> targetType) {
        if (this.targetType != targetType) {
            ...
            if (this.field == null && PropertySubstitute.log.isLoggable(Logger.Level.WARNING)) {
                PropertySubstitute.log.warn(String.format("Failed to find field for %s.%s", targetType.getName(), this.getName()));
            }
    ...
    private Method discoverMethod(final Class<?> type, final String name, final Class<?>... params) {
        ...
        if (PropertySubstitute.log.isLoggable(Logger.Level.WARNING)) {
            PropertySubstitute.log.warn(String.format("Failed to find [%s(%d args)] for %s.%s", name, params.length, this.targetType.getName(), this.getName()));
        }
        return null;
    }

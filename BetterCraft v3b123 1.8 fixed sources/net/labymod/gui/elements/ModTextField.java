...
public class ModTextField extends Gui
{
    ...
    public ModTextField(final int componentId, final FontRenderer fontrenderer, final int x, final int y, final int par5Width, final int par6Height) {
        this.text = "";
    ...
    public boolean mouseClicked(final int p_146192_1_, final int p_146192_2_, final int p_146192_3_) {
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            ...
            final String s = this.fontRendererInstance.trimStringToWidth(theText, this.getWidth());
            ...
            if (s.length() > 0) {
                ...
                j2 += this.fontRendererInstance.getStringWidth(this.visualColorForText(s2, true));
            }
            ...
            if (s.length() > 0 && flag && j < s.length()) {
                j2 += this.fontRendererInstance.getStringWidth(this.visualColorForText(s.substring(j), false));
            }
            this.fontRendererInstance.drawStringWithShadow(this.visualColorForText(s, false), (float)l, (float)i2, i);
            ...
            if (placeHolder) {
                this.drawString(this.fontRendererInstance, this.placeHolder, k2, i2, Color.LIGHT_GRAY.getRGB());
            }
        }
    }
    ...
    public void drawColorBar(final int mouseX, final int mouseY) {
        if (this.colorBarEnabled) {
            ...
            for (int length = (values = ModColor.values()).length, i = 0; i < length; ++i) {
                ...
                boolean hovered = mouseX > pX - 4 && mouseX < pX + 4 && mouseY > pY - 1 && mouseY < pY + 9;
                if (hovered) {
                    this.hoveredModColor = color;
                }
                if (this.colorAtCursor != null && this.colorAtCursor.equals(new StringBuilder().append(color.getColorChar()).toString())) {
                    hovered = true;
                }
                Gui.drawRect(pX - 4, pY - 1, pX + 4, pY + 9, hovered ? Integer.MAX_VALUE : ModColor.toRGB(120, 120, 120, 120));
    ...
    private String visualColorForText(final String text, final boolean saveCursorColor) {
        ...
        for (int i = 0; i < text.length(); ++i) {
            ...
            if (c == '&' && i != text.length() - 1) {
                ...
                if (foundColor) {
                    foundColor = false;
                    coloredString = String.valueOf(coloredString) + "\u00c2§" + c + '&';
                    if (saveCursorColor) {
                        this.colorAtCursor = new StringBuilder().append(c).toString();
                    }
                }
                coloredString = String.valueOf(coloredString) + c;
            }
        }

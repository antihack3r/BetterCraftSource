    ...
    protected void renderLivingLabel(final T entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        ...
        if (d0 <= maxDistance * maxDistance) {
            ...
            if (entityIn instanceof EntityPlayer) {
                try {
                    final String value = String.valueOf(String.valueOf(Math.round(((EntityPlayer)entityIn).getHealth()) / 2) + " §fhp §8/ §r" + Math.round(entityIn.getDistanceToEntity(Minecraft.getMinecraft().thePlayer)) + " m §8/ §r" + Minecraft.getMinecraft().getNetHandler().getPlayerInfo(entityIn.getUniqueID()).getResponseTime() + " ms");
                    fontrenderer.drawString(value, -fontrenderer.getStringWidth(value) / 2, i - 20, -1);

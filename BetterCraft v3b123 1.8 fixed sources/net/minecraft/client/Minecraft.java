...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public Minecraft(final GameConfiguration gameConfig) {
        ...
        Minecraft.logger.info("(Session ID is #)");
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        BetterCraft.getInstance();
        final StringBuilder append2 = append.append(BetterCraft.clientVersion).append(" ");
        BetterCraft.getInstance();
        Display.setTitle(append2.append(BetterCraft.clientBuild).append(" | Minecraft ").append(getMinecraft().getVersion()).toString());
    }
    ...
    public static void setIcon(final String icon16, final String icon32) {
        InputStream inputstream = null;
        InputStream inputstream2 = null;
        try {
            inputstream = getMinecraft().mcDefaultResourcePack.getInputStream(new ResourceLocation(icon16));
            inputstream2 = getMinecraft().mcDefaultResourcePack.getInputStream(new ResourceLocation(icon32));
    ...
    private void setWindowIcon() {
        if (Util.getOSType() != Util.EnumOS.OSX) {
            setIcon("client/icons/icon16x16.png", "client/icons/icon32x32.png");
        }
    }
    ...
    public void shutdownMinecraftApplet() {
        Label_0154: {
            try {
                ...
                final int asInt = Manager.readJsonFromUrl().getAsJsonObject().get("Version").getAsInt();
                BetterCraft.getInstance();
                if (asInt <= BetterCraft.clientVersionCheck) {
                    final int asInt2 = Manager.readJsonFromUrl().getAsJsonObject().get("Version").getAsInt();
                    BetterCraft.getInstance();
                    if (asInt2 != BetterCraft.clientVersionCheck) {
                        break Label_0154;
                    }
                    final int asInt3 = Manager.readJsonFromUrl().getAsJsonObject().get("Build").getAsInt();
                    BetterCraft.getInstance();
                    if (asInt3 <= BetterCraft.clientBuildCheck) {
                        break Label_0154;
                    }
                }
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (this.currentScreen != null) {
                    ...
                    if (getMinecraft().gameSettings.CLIENT_GUI_MOD_POS.isPressed()) {
                        HUDManager.getInstance().openConfigScreen();
                    }
                    if (getMinecraft().gameSettings.CLIENT_GUI_NBTEDIT.isPressed() && this.thePlayer.getHeldItem().hasTagCompound()) {
                        this.displayGuiScreen(new GuiEditNBTTree(this.thePlayer.getEntityId(), this.thePlayer.getHeldItem().stackTagCompound));
                    }
                    if (getMinecraft().gameSettings.CLIENT_GUI_MOD_STATE.isPressed()) {
                        getMinecraft().displayGuiScreen(new GuiRShift(this.currentScreen));
                    }
                    if (getMinecraft().gameSettings.CLIENT_CREATIVE_TAB.isPressed()) {
                        TabCreativeOwnItems.saveItemstack(this.thePlayer.getHeldItem());
                        final GuiNewChat chatGUI = getMinecraft().ingameGUI.getChatGUI();
                        BetterCraft.getInstance();
                        chatGUI.printChatMessage(new ChatComponentText(String.valueOf(BetterCraft.clientPrefix) + "Item §f'" + this.thePlayer.getHeldItem().getDisplayName() + "' §7saved in Own Items"));
                    }
                }
                if (Keyboard.getEventKeyState()) {

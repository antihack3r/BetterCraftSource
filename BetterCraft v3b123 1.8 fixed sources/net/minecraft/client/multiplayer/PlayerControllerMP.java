...
public class PlayerControllerMP
{
    ...
    public boolean onPlayerRightClick(final EntityPlayerSP player, final WorldClient worldIn, final ItemStack heldStack, final BlockPos hitPos, final EnumFacing side, final Vec3 hitVec) {
        ...
        if (this.currentGameType.isCreative()) {
            ...
            final Block block = Block.getBlockFromItem(heldStack.getItem());
            if (Objects.nonNull(block)) {
                final IProperty damageProperty = block.getBlockState().getProperties().stream().filter(property -> property.getName().equals("damage")).findFirst().orElse(null);
                final IProperty variantProperty = block.getBlockState().getProperties().stream().filter(property -> property.getName().equals("variant")).findFirst().orElse(null);
                if ((Objects.nonNull(damageProperty) && !damageProperty.getAllowedValues().contains(i)) || (Objects.nonNull(variantProperty) && !variantProperty.getAllowedValues().contains(i))) {
                    return false;
                }
            }
    ...
    public void attackEntity(final EntityPlayer playerIn, final Entity targetEntity) {
        ModSkin.setCurrentEntity(targetEntity);
        this.syncCurrentPlayItem();

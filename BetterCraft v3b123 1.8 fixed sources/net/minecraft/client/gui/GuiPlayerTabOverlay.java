...
public class GuiPlayerTabOverlay extends Gui
{
    ...
    public void renderPlayerlist(final int width, final Scoreboard scoreboardIn, final ScoreObjective scoreObjectiveIn) {
        ...
        for (final NetworkPlayerInfo networkplayerinfo : list) {
            int k = this.mc.fontRendererObj.getStringWidth(this.getPlayerName(networkplayerinfo)) + 25;
            i = Math.max(i, k);
        ...
        for (int k3 = 0; k3 < l3; ++k3) {
            ...
            final NetworkPlayerInfo networkPlayer = list.get(k3);
            final boolean isBedrock = networkPlayer.getGameProfile().getName().startsWith("!") || networkPlayer.getGameProfile().getName().startsWith(".");
            final boolean isLabyMod = LabyMod.getInstance().getUserManager().getFamiliarManager().isFamiliar(networkPlayer.getGameProfile().getId());
            Gui.drawRect(j4, k4, j4 + i3, k4 + 8, IRC.getInstance().isUserConnected(networkPlayer.getGameProfile().getName()) ? Integer.MAX_VALUE : (isLabyMod ? -1439786242 : (isBedrock ? -41333432 : Integer.MIN_VALUE)));
            ...
            if (k3 < list.size()) {
                ...
                else if (networkplayerinfo2.getGameType() == WorldSettings.GameType.CREATIVE) {
                    s4 = "§8[§f!§8]§r " + s4;

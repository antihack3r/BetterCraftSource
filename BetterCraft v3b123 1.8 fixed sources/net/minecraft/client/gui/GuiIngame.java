...
public class GuiIngame extends Gui
{
    ...
    protected void renderTooltip(final ScaledResolution sr, final float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ASMEventManager.call(new RenderEvents.ToolTip(TypePrePost.PRE, sr, partialTicks));
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            ...
            if (GuiUISettings.enabledUI[0]) {
                if (!(this.mc.currentScreen instanceof GuiChat)) {
                    try {
                        ...
                        this.drawString(this.mc.fontRendererObj, "§fFPS: §r" + Minecraft.debugFPS + " §fPing: §r" + String.valueOf(this.mc.getNetHandler().getPlayerInfo(this.mc.thePlayer.getUniqueID()).getResponseTime()), 5, sr.getScaledHeight() - 22, ColorUtils.rainbowEffect());
                        this.drawString(this.mc.fontRendererObj, "§fX: §r" + Math.round(this.mc.thePlayer.posX) + " §fY: §r" + Math.round(this.mc.thePlayer.posY) + " §fZ: §r" + Math.round(this.mc.thePlayer.posZ), 5, sr.getScaledHeight() - 12, ColorUtils.rainbowEffect());
                        final DateFormat dff = new SimpleDateFormat("HH:mm:ss");
                        final Date todayy = Calendar.getInstance().getTime();
                        final String time = dff.format(todayy);
                        final DateFormat df = new SimpleDateFormat("dd/MM/yyyy");
                        final Date today = Calendar.getInstance().getTime();
                        final String date = df.format(today);
                        this.drawString(this.mc.fontRendererObj, "§fTime: §r" + time, sr.getScaledWidth() - 72, sr.getScaledHeight() - 22, ColorUtils.rainbowEffect());
                        this.drawString(this.mc.fontRendererObj, "§fDate: §r" + date, sr.getScaledWidth() - 72, sr.getScaledHeight() - 12, ColorUtils.rainbowEffect());
                    }

...
public class GuiNewChat extends Gui
{
                        ...
                        if (j2 < 200 || flag) {
                            ...
                            if (l2 > 3) {
                                ...
                                if (!GuiUISettings.enabledBackgrounds[2]) {
                                    Gui.drawRect(i3, j3 - 1, i3 + l + 4, j3 - 10, l2 / 2 << 24);
                                }
                                final String s = chatline.getChatComponent().getFormattedText();
                                GlStateManager.enableBlend();
                                this.mc.fontRendererObj.drawStringWithShadow(s, (float)i3, (float)(j3 - 10), 16777215 + (l2 << 24));
    ...
    public void printChatMessage(final IChatComponent chatComponent) {
        ...
        this.printChatMessageWithOptionalDeletion(new ChatComponentText(String.format("%s%s%s", EnumChatFormatting.GRAY, new SimpleDateFormat("[HH:mm:ss] ").format(System.currentTimeMillis()), EnumChatFormatting.RESET)).appendSibling(chatComponent), 0);
    }
    ...
    private void setChatLine(final IChatComponent chatComponent, final int chatLineId, final int updateCounter, final boolean displayOnly) {
        ...
        for (final IChatComponent ichatcomponent : list) {
            ...
            if (GuiUISettings.enabledAnimations[1]) {
                final ChatLine line = new ChatLine(updateCounter, new ChatComponentText(""), chatLineId);
                TextAnimationUtils.writeTextAnimation(ichatcomponent.getUnformattedText(), 20L, value -> chatLine.setChatComponent(new ChatComponentText(value)), value -> chatLine2.setChatComponent(chatComponent2));

...
public abstract class GuiSlot
{
    ...
    public void drawScreen(final int mouseXIn, final int mouseYIn, final float p_148128_3_) {
        if (this.field_178041_q) {
            ...
            startScissor(0.0f, (float)this.top, (float)this.width, (float)this.bottom);
            ...
            worldrenderer.pos(this.left, this.top + i2, 0.0).tex(0.0, 1.0).color(255, 255, 255, 255).endVertex();
            worldrenderer.pos(this.right, this.top + i2, 0.0).tex(1.0, 1.0).color(255, 255, 255, 255).endVertex();
            worldrenderer.pos(this.right, this.top, 0.0).tex(1.0, 0.0).color(r, g, b, a).endVertex();
            worldrenderer.pos(this.left, this.top, 0.0).tex(0.0, 0.0).color(r, g, b, a).endVertex();
            tessellator.draw();
            worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            worldrenderer.pos(this.left, this.bottom, 0.0).tex(0.0, 1.0).color(r, g, b, a).endVertex();
            worldrenderer.pos(this.right, this.bottom, 0.0).tex(1.0, 1.0).color(r, g, b, a).endVertex();
            worldrenderer.pos(this.right, this.bottom - i2, 0.0).tex(1.0, 0.0).color(255, 255, 255, 255).endVertex();
            worldrenderer.pos(this.left, this.bottom - i2, 0.0).tex(0.0, 0.0).color(255, 255, 255, 255).endVertex();
            tessellator.draw();
    ...
    protected void drawSelectionBox(final int p_148120_1_, final int p_148120_2_, final int mouseXIn, final int mouseYIn) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (this.showSelectionBox && this.isSelected(j)) {
                ...
                worldrenderer.pos(i2, k - 2, 0.0).tex(0.0, 0.0).color(r, g, b, a).endVertex();
                ...
                worldrenderer.pos(i2 + 1, k - 1, 0.0).tex(0.0, 0.0).color(80, 80, 80, 255).endVertex();
                tessellator.draw();
                GlStateManager.enableTexture2D();
            }

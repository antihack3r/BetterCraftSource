...
public class EventManager
{
    ...
    public static void call(final IEvent event) {
        ...
        for (final EventExecutor listener : eventListener) {
            try {
                ...
                EventManager.ERROR_LISTENER.forEach(errorListener -> errorListener.catchException(t));
            }
    ...
    public static void unregister(final Object listener) {
        for (final Map.Entry<Class<? extends IEvent>, List<EventExecutor>> entry : EventManager.EVENT_LISTENER.entrySet()) {
            entry.getValue().removeIf(eventExecutor -> eventExecutor.getEventListener().equals(o) || (eventExecutor.getEventListener() instanceof ReflectedEventListener && ((ReflectedEventListener)eventExecutor.getEventListener()).getCallInstance().equals(o)));
        }
    }

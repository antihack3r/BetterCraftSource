...
class Utils
{
    ...
    public static void drawDarkBackground(final int top, final int left, final int bottom, final int right) {
        ...
        wr.pos(0.0, bottom, 0.0).tex(0.0f / textureSize, bottom / textureSize).color(32, 32, 32, 255).endVertex();
        wr.pos(right, bottom, 0.0).tex(right / textureSize, bottom / textureSize).color(32, 32, 32, 255).endVertex();
        wr.pos(right, top, 0.0).tex(right / textureSize, top / textureSize).color(32, 32, 32, 255).endVertex();
        wr.pos(left, top, 0.0).tex(left / textureSize, top / textureSize).color(32, 32, 32, 255).endVertex();
        t.draw();
    }
    ...
    public static void drawBorder(final int topMargin, final int bottomMargin, final int top, final int left, final int bottom, final int right) {
        ...
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        wr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        wr.pos(left, upperBoxEnd, 0.0).tex(0.0, upperBoxEnd / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(right, upperBoxEnd, 0.0).tex(right / textureSize, upperBoxEnd / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(right, top, 0.0).tex(right / textureSize, top / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(left, top, 0.0).tex(0.0, top / textureSize).color(64, 64, 64, 255).endVertex();
        t.draw();
        final int lowerBoxStart = bottom - bottomMargin;
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        wr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        wr.pos(left, bottom, 0.0).tex(0.0, bottom / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(right, bottom, 0.0).tex(right / textureSize, bottom / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(right, lowerBoxStart, 0.0).tex(right / textureSize, lowerBoxStart / textureSize).color(64, 64, 64, 255).endVertex();
        wr.pos(left, lowerBoxStart, 0.0).tex(0.0, lowerBoxStart / textureSize).color(64, 64, 64, 255).endVertex();
        t.draw();
        ...
        wr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        wr.pos(left, upperBoxEnd + padding, 0.0).tex(0.0, 1.0).color(0, 0, 0, 0).endVertex();
        wr.pos(right, upperBoxEnd + padding, 0.0).tex(1.0, 1.0).color(0, 0, 0, 0).endVertex();
        wr.pos(right, upperBoxEnd, 0.0).tex(1.0, 0.0).color(0, 0, 0, 255).endVertex();
        wr.pos(left, upperBoxEnd, 0.0).tex(0.0, 0.0).color(0, 0, 0, 255).endVertex();
        t.draw();
        wr.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        wr.pos(left, lowerBoxStart, 0.0).tex(0.0, 1.0).color(0, 0, 0, 255).endVertex();
        wr.pos(right, lowerBoxStart, 0.0).tex(1.0, 1.0).color(0, 0, 0, 255).endVertex();
        wr.pos(right, lowerBoxStart - padding, 0.0).tex(1.0, 0.0).color(0, 0, 0, 0).endVertex();
        wr.pos(left, lowerBoxStart - padding, 0.0).tex(0.0, 0.0).color(0, 0, 0, 0).endVertex();
        t.draw();

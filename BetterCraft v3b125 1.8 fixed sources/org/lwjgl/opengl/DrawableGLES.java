...
implements DrawableLWJGL {
     ...
     */
    public void setPixelFormat(PixelFormatLWJGL pf2) throws LWJGLException {
        Object object = GlobalLock.lock;
        synchronized (object) {
            this.pixel_format = (PixelFormat)pf2;
        }
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    public void initialize(long window, long display_id, int eglSurfaceType, PixelFormat pf2) throws LWJGLException {
        Object object = GlobalLock.lock;
        synchronized (object) {
            ...
            EGLConfig[] configs = eglDisplay.chooseConfig(pf2.getAttribBuffer(eglDisplay, eglSurfaceType, attribs), null, BufferUtils.createIntBuffer(1));
            if (configs.length == 0) {
                throw new LWJGLException("No EGLConfigs found for the specified PixelFormat.");
            }
            EGLConfig eglConfig = pf2.getBestMatch(configs);
            EGLSurface eglSurface = eglDisplay.createWindowSurface(eglConfig, window, null);
            pf2.setSurfaceAttribs(eglSurface);

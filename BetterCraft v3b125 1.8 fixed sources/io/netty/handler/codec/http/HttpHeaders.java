...
implements Iterable<Map.Entry<String, String>> {
    ...
    public static void setKeepAlive(HttpMessage message, boolean keepAlive) {
        ...
        if (message.getProtocolVersion().isKeepAliveDefault()) {
            if (keepAlive) {
                h2.remove(CONNECTION_ENTITY);
            } else {
                h2.set(CONNECTION_ENTITY, (Object)CLOSE_ENTITY);
            }
        } else if (keepAlive) {
            h2.set(CONNECTION_ENTITY, (Object)KEEP_ALIVE_ENTITY);
        } else {
            h2.remove(CONNECTION_ENTITY);
        }
    }
    ...
    private static int getWebSocketContentLength(HttpMessage message) {
        ...
        if (message instanceof HttpRequest) {
            ...
            if (HttpMethod.GET.equals(req.getMethod()) && h2.contains(SEC_WEBSOCKET_KEY1_ENTITY) && h2.contains(SEC_WEBSOCKET_KEY2_ENTITY)) {
                return 8;
            }
        } else if (message instanceof HttpResponse && (res = (HttpResponse)message).getStatus().code() == 101 && h2.contains(SEC_WEBSOCKET_ORIGIN_ENTITY) && h2.contains(SEC_WEBSOCKET_LOCATION_ENTITY)) {
            return 16;
        }
        return -1;
    }
    ...
    public static void removeTransferEncodingChunked(HttpMessage m2) {
        List<String> values = m2.headers().getAll(TRANSFER_ENCODING_ENTITY);
        if (values.isEmpty()) {
            return;
        }
        ...
        if (values.isEmpty()) {
            m2.headers().remove(TRANSFER_ENCODING_ENTITY);
        } else {
            m2.headers().set(TRANSFER_ENCODING_ENTITY, values);
        }
    }

    public static void setTransferEncodingChunked(HttpMessage m2) {
        HttpHeaders.addHeader(m2, TRANSFER_ENCODING_ENTITY, (Object)CHUNKED_ENTITY);
        HttpHeaders.removeHeader(m2, CONTENT_LENGTH_ENTITY);
    }

    public static boolean isContentLengthSet(HttpMessage m2) {
        return m2.headers().contains(CONTENT_LENGTH_ENTITY);
    }
    ...
    public boolean contains(String name, String value, boolean ignoreCaseValue) {
        ...
        for (String v2 : values) {
            if (!(ignoreCaseValue ? HttpHeaders.equalsIgnoreCase(v2, value) : v2.equals(value))) continue;
            return true;
        }
        return false;
    }

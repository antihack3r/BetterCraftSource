...
extends TypeAdapter<Style> {
    ...
    @Override
    public Style read(JsonReader in2) throws IOException {
        in2.beginObject();
        Style.Builder style = Style.style();
        while (in2.hasNext()) {
            String fieldName = in2.nextName();
            if (fieldName.equals("font")) {
                style.font((Key)this.gson.fromJson(in2, SerializerFactory.KEY_TYPE));
                continue;
            }
            if (fieldName.equals("color")) {
                TextColorWrapper color = (TextColorWrapper)this.gson.fromJson(in2, SerializerFactory.COLOR_WRAPPER_TYPE);
                if (color.color != null) {
            ...
            if (TextDecoration.NAMES.keys().contains(fieldName)) {
                style.decoration(TextDecoration.NAMES.value(fieldName), this.readBoolean(in2));
                continue;
            }
            if (fieldName.equals("insertion")) {
                style.insertion(in2.nextString());
                continue;
            }
            if (fieldName.equals("clickEvent")) {
                ...
                String value = null;
                while (in2.hasNext()) {
                    String clickEventField = in2.nextName();
                    if (clickEventField.equals("action")) {
                        action = (ClickEvent.Action)((Object)this.gson.fromJson(in2, SerializerFactory.CLICK_ACTION_TYPE));
                        continue;
                    }
                    if (clickEventField.equals("value")) {
                        value = in2.peek() == JsonToken.NULL ? null : in2.nextString();
                        continue;
                    }
                    in2.skipValue();
                }
            ...
            if (fieldName.equals("hoverEvent")) {
                ...
                JsonObject hoverEventObject = (JsonObject)this.gson.fromJson(in2, (Type)((Object)JsonObject.class));
    ...
    private boolean readBoolean(JsonReader in2) throws IOException {

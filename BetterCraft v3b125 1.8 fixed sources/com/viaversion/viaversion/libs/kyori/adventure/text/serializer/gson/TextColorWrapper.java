...
final class TextColorWrapper {
    ...
    extends TypeAdapter<TextColorWrapper> {
        ...
        @Override
        public TextColorWrapper read(JsonReader in2) throws IOException {
            boolean reset;
            String input = in2.nextString();
            TextColor color = TextColorSerializer.fromString(input);
            TextDecoration decoration = TextDecoration.NAMES.value(input);
            boolean bl2 = reset = decoration == null && input.equals("reset");
            if (color == null && decoration == null && !reset) {
                throw new JsonParseException("Don't know how to parse " + input + " at " + in2.getPath());
            }

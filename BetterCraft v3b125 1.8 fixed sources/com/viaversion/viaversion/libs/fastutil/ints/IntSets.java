...
public final class IntSets {
    ...
    public static IntSet fromTo(final int from, final int to2) {
        return new AbstractIntSet(){

            @Override
            public boolean contains(int x2) {
                return x2 >= from && x2 < to2;
            }

            @Override
            public IntIterator iterator() {
                return IntIterators.fromTo(from, to2);
            }

            @Override
            public int size() {
                long size = (long)to2 - (long)from;
    ...
    public static IntSet to(final int to2) {
        return new AbstractIntSet(){
            ...
            @Override
            public IntIterator iterator() {
                return IntIterators.fromTo(Integer.MIN_VALUE, to2);
            }

            @Override
            public int size() {
                long size = (long)to2 - Integer.MIN_VALUE;
                return size >= 0L && size <= Integer.MAX_VALUE ? (int)size : Integer.MAX_VALUE;
            }
        };
    }

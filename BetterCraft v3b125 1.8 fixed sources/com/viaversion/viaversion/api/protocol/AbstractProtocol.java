...
implements Protocol<CU, CM, SM, SU> {
    ...
    protected void registerConfigurationChangeHandlers() {
        ...
        if ((finishConfigurationPacket = this.serverboundFinishConfigurationPacket()) != null) {
            int id2 = finishConfigurationPacket.getId();
            this.registerServerbound(State.CONFIGURATION, id2, id2, this.setClientStateHandler(State.PLAY));
        }
        if ((clientboundFinishConfigurationPacket = this.clientboundFinishConfigurationPacket()) != null) {
            int id3 = clientboundFinishConfigurationPacket.getId();
            this.registerClientbound(State.CONFIGURATION, id3, id3, this.setServerStateHandler(State.PLAY));
        }
    }
    ...
    public void transform(Direction direction, State state, PacketWrapper packetWrapper) throws Exception {
        ...
        if (handler != null) {
            try {
                ...
                e3.addSource(handler.getClass());
                this.throwRemapError(direction, state, unmappedId, packetWrapper.getId(), e3);
                return;
            }
            catch (Exception e4) {
                InformativeException ex2 = new InformativeException(e4);
                ex2.addSource(handler.getClass());
                this.throwRemapError(direction, state, unmappedId, packetWrapper.getId(), ex2);
                return;
            }
    ...
    protected void throwRemapError(Direction direction, State state, int unmappedPacketId, int mappedPacketId, InformativeException e2) throws InformativeException {
        Object packetType;
    ...
    public static String toNiceHex(int id2) {
        String hex = Integer.toHexString(id2).toUpperCase();

...
implements MappingData {
    ...
    public void load() {
        ...
        if (unmappedIdentifierData != null && mappedIdentifierData != null) {
            ...
            if (unmappedParticles != null && mappedParticles != null) {
                ...
                List<String> identifiers = unmappedParticles.getValue().stream().map(t2 -> (String)t2.getValue()).collect(Collectors.toList());
                List<String> mappedIdentifiers = mappedParticles.getValue().stream().map(t2 -> (String)t2.getValue()).collect(Collectors.toList());
    ...
    public int getNewBlockStateId(int id2) {
        return this.checkValidity(id2, this.blockStateMappings.getNewId(id2), "blockstate");
    }

    @Override
    public int getNewBlockId(int id2) {
        return this.checkValidity(id2, this.blockMappings.getNewId(id2), "block");
    }

    @Override
    public int getNewItemId(int id2) {
        return this.checkValidity(id2, this.itemMappings.getNewId(id2), "item");
    }

    @Override
    public int getOldItemId(int id2) {
        return this.itemMappings.inverse().getNewIdOrDefault(id2, 1);
    }

    @Override
    public int getNewParticleId(int id2) {
        return this.checkValidity(id2, this.particleMappings.getNewId(id2), "particles");
    }
    ...
    protected int checkValidity(int id2, int mappedId, String type) {
        if (mappedId == -1) {
            if (!Via.getConfig().isSuppressConversionWarnings()) {
                this.getLogger().warning(String.format("Missing %s %s for %s %s %d", this.mappedVersion, type, this.unmappedVersion, type, id2));
            }
            return 0;
        }
        return mappedId;

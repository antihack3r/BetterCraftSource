...
public final class ConnectionData {
    ...
    public static void connectBlocks(UserConnection user, Chunk chunk) {
        ...
        for (int s2 = 0; s2 < chunk.getSections().length; ++s2) {
            ...
            for (int p2 = 0; p2 < blocks.size(); ++p2) {
                int id2 = blocks.idByIndex(p2);
                if (!ConnectionData.connects(id2)) continue;
                willConnect = true;
                break;
            }
            ...
            for (int idx = 0; idx < 4096; ++idx) {
                ...
                ConnectionHandler handler = ConnectionData.getConnectionHandler(id3);
                if (handler == null || (connectedId = handler.connect(user, position = new Position(xOff + ChunkSection.xFromIndex(idx), yOff + ChunkSection.yFromIndex(idx), zOff + ChunkSection.zFromIndex(idx)), id3)) == id3) continue;
    ...
    public static void init() {
        ...
        for (int id2 = 0; id2 < blockStates.size(); ++id2) {
            String key = (String)((Tag)blockStates.get(id2)).getValue();
            KEY_TO_ID.put(key, id2);
        }
        ...
        if (!Via.getConfig().isReduceBlockStorageMemory()) {
            ...
            for (Tag blockTag : blockConnectionMappings) {
                ...
                for (int id3 : idsTag.getValue()) {
                    blockConnectionData.put(id3, blockData);
                }
            }
        }
    ...
    public static final class NeighbourUpdater {
        ...
        private void updateBlock(int x2, int y2, int z2, List<BlockChangeRecord1_8> records) {
            ...
            Position pos = new Position(x2, y2, z2);

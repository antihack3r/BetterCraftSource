        ...
        this.registerClientbound(ClientboundPackets1_9.CHUNK_DATA, (PacketWrapper wrapper) -> {
            ...
            for (int s2 = 0; s2 < chunk.getSections().length; ++s2) {
                ...
                for (int idx = 0; idx < 4096; ++idx) {
                    ...
                    if (!FakeTileEntity.isTileEntity(id2)) continue;
                    tags.add(FakeTileEntity.createTileEntity(ChunkSection.xFromIndex(idx) + (chunk.getX() << 4), ChunkSection.yFromIndex(idx) + (s2 << 4), ChunkSection.zFromIndex(idx) + (chunk.getZ() << 4), id2));
                }
            }
        });

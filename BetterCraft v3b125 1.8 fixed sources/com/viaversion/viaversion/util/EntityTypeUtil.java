...
public final class EntityTypeUtil {
    ...
    public static <T extends EntityType> void initialize(T[] values, EntityType[] typesToFill, Protocol<?, ?, ?, ?> protocol, EntityIdSetter<T> idSetter) {
        for (T type : values) {
            ...
            int id2 = protocol.getMappingData().getEntityMappings().mappedId(type.identifier());
            Preconditions.checkArgument(id2 != -1, "Entity type %s has no id", type.identifier());
            idSetter.setId(type, id2);
            typesToFill[id2] = type;
        }
    }

...
public final class DumpUtil {
    public static CompletableFuture<String> postDump(@Nullable UUID playerToSample) {
        ...
        Via.getPlatform().runAsync(() -> {
            ...
            try {
                ...
                Via.getPlatform().getLogger().log(Level.SEVERE, "Error when opening connection to ViaVersion dump service", e2);
                result.completeExceptionally(new DumpException(DumpErrorType.CONNECTION, e2));
                return;
            }
            try {
                String rawOutput;
                ...
                Via.getPlatform().getLogger().log(Level.SEVERE, "Error when posting ViaVersion dump", e3);
                result.completeExceptionally(new DumpException(DumpErrorType.POST, e3));
    ...
    public static String urlForId(String id2) {
        return String.format("https://dump.viaversion.com/%s", id2);
    }

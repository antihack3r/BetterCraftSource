    ...
    public Item handleItemToClient(Item item) {
        ...
        if (tag.contains("ench") || tag.contains("StoredEnchantments")) {
            ...
            for (Tag ench : new ArrayList(enchTag.getValue())) {
                String s2;
                short id2 = ((NumberTag)((CompoundTag)ench).get("id")).asShort();
        ...
        if (item.identifier() == 383 && item.data() == 0) {
            ...
            if (tag.contains("EntityTag") && (entityTag = (CompoundTag)tag.get("EntityTag")).contains("id")) {
                StringTag id3 = (StringTag)entityTag.get("id");
                if (ItemRewriter.ENTITY_NAME_TO_ID.containsKey(id3.getValue())) {
                    data = ItemRewriter.ENTITY_NAME_TO_ID.get(id3.getValue());
                } else if (display == null) {
                    display = new CompoundTag();
                    tag.put("display", display);
                    viaVersionTag.put("noDisplay", new ByteTag());
                    display.put("Name", new StringTag("\u00a7rSpawn " + id3.getValue()));
                }
            }

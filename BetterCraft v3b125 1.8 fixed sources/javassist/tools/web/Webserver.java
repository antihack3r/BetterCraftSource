...
public class Webserver {
    ...
    public void run() {
        ...
        while (true) {
            try {
                while (true) {
                    ServiceThread th2 = new ServiceThread(this, this.socket.accept());
                    th2.start();
                }
            }
    ...
    final void process(Socket clnt) throws IOException {
        BufferedInputStream in2 = new BufferedInputStream(clnt.getInputStream());
        String cmd = this.readLine(in2);
        this.logging(clnt.getInetAddress().getHostName(), new Date().toString(), cmd);
        while (this.skipLine(in2) > 0) {
        }
        BufferedOutputStream out = new BufferedOutputStream(clnt.getOutputStream());
        try {
            this.doReply(in2, out, cmd);
        }
    ...
    private int skipLine(InputStream in2) throws IOException {
        int c2;
        int len = 0;
        while ((c2 = in2.read()) >= 0 && c2 != 13) {
            ++len;

...
public class ServerAddress {
    ...
    public static ServerAddress resolveAddress(String serverIP) {
        ...
        if (serverIP.startsWith("[") && (i2 = serverIP.indexOf("]")) > 0) {
            String s2 = serverIP.substring(1, i2);
            String s1 = serverIP.substring(i2 + 1).trim();
            if (s1.startsWith(":") && s1.length() > 0) {
                s1 = s1.substring(1);
                astring = new String[]{s2, s1};
            } else {
        ...
        String s2 = astring[0];
        int j2 = astring.length > 1 ? ServerAddress.getInt(astring[1], 25565) : 25565;
        int n2 = j2;
        if (j2 == 25565) {
            String[] astring1 = ServerAddress.getServerAddress(s2);
            s2 = astring1[0];

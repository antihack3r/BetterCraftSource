...
public class TeamSpeakChannel {
    private static final List<TeamSpeakChannel> channels = new ArrayList<TeamSpeakChannel>();
    ...
    protected static List<TeamSpeakChannel> getChannels() {
        ArrayList<TeamSpeakChannel> list = new ArrayList<TeamSpeakChannel>();
    ...
    public String getChannelCodecName() {
        return String.valueOf(this.channel_codec).replace("0", "Speex Narrowband").replace("1", "Speex Wideband").replace("2", "Speex Ultra-Wideband").replace("3", "CELT Mono").replace("4", "Opus Voice").replace("5", "Opus Music");
    }
    ...
    public static boolean contains(int id2) {
        for (TeamSpeakChannel teamspeakchannel : channels) {
            if (teamspeakchannel.getChannelId() != id2) continue;
            return true;
        }
        return false;
    }

    public static void deleteChannel(TeamSpeakChannel c2) {
        channels.remove(c2);
    }

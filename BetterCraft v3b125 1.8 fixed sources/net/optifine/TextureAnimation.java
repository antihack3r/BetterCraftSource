...
public class TextureAnimation {
    ...
    public TextureAnimation(String texFrom, byte[] srcData, String texTo, ResourceLocation locTexTo, int dstX, int dstY, int frameWidth, int frameHeight, Properties props) {
        ...
        if (this.interpolate) {
            this.interpolateData = GLAllocation.createDirectByteBuffer(i2);
        }
    }
    ...
    public void updateTexture() {
        if (this.valid) {
            ...
            boolean bl2 = this.active = SmartAnimations.isActive() ? SmartAnimations.isTextureRendered(this.dstTextId) : true;
            if (this.nextFrame() && this.active) {
                int i2;
                int j2 = this.frameWidth * this.frameHeight * 4;
                TextureAnimationFrame textureanimationframe = this.getCurrentFrame();
                if (textureanimationframe != null && (i2 = j2 * textureanimationframe.index) + j2 <= this.imageData.limit()) {
    ...
    private void updateTextureInerpolate(TextureAnimationFrame frame1, TextureAnimationFrame frame2, double dd2) {
        int k2;
        int i2 = this.frameWidth * this.frameHeight * 4;
        int j2 = i2 * frame1.index;
        if (j2 + i2 <= this.imageData.limit() && (k2 = i2 * frame2.index) + i2 <= this.imageData.limit()) {
            this.interpolateData.clear();
            int l2 = 0;
            while (l2 < i2) {
                int i1 = this.imageData.get(j2 + l2) & 0xFF;
                int j1 = this.imageData.get(k2 + l2) & 0xFF;
                int k1 = this.mix(i1, j1, dd2);
                byte b0 = (byte)k1;
    ...
    private int mix(int col1, int col2, double k2) {
        return (int)((double)col1 * (1.0 - k2) + (double)col2 * k2);
    }

    ...
    public float eval() {
        ...
        switch (this.type) {
            case SMOOTH: {
                ...
                float f3 = FunctionFloat.evalFloat(aiexpression, 0);
                float f1 = aiexpression.length > 1 ? FunctionFloat.evalFloat(aiexpression, 1) : 1.0f;
                float f4 = f2 = aiexpression.length > 2 ? FunctionFloat.evalFloat(aiexpression, 2) : f1;
                if (this.smoothId < 0) {
                    this.smoothId = Smoother.getNextId();
                }
                float f32 = Smoother.getSmoothValue(this.smoothId, f3, f1, f2);
                return f32;
            }
        }

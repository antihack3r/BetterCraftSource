...
public class PlayerPackets
{
    public static void register(final Protocol1_7_6_10TO1_8 protocol) {
        ...
        ((AbstractProtocol<ClientboundPackets1_8, ClientboundPackets1_7, S1, S2>)protocol).registerClientbound(ClientboundPackets1_8.CAMERA, null, new PacketRemapper() {
            @Override
            public void registerMap() {
                this.handler(packetWrapper -> {
                    packetWrapper.cancel();
        ...
        ((AbstractProtocol<ClientboundPackets1_8, ClientboundPackets1_7, S1, S2>)protocol).registerClientbound(ClientboundPackets1_8.TITLE, null, new PacketRemapper() {
            @Override
            public void registerMap() {
                this.handler(packetWrapper -> {
                    packetWrapper.cancel();
        ...
        ((AbstractProtocol<ClientboundPackets1_8, ClientboundPackets1_7, S1, S2>)protocol).registerClientbound(ClientboundPackets1_8.RESOURCE_PACK, null, new PacketRemapper() {
            @Override
            public void registerMap() {
                this.handler(packetWrapper -> {
                    packetWrapper.cancel();
                    final PacketWrapper response = packetWrapper.create(ServerboundPackets1_8.RESOURCE_PACK_STATUS);
        ...
        ((AbstractProtocol<C1, C2, S1, ServerboundPackets1_7>)protocol).registerServerbound(ServerboundPackets1_7.PLUGIN_MESSAGE, new PacketRemapper() {
            ...
            public void registerMap() {
                ...
                this.handler(packetWrapper -> {
                    final String channel = packetWrapper.get(Type.STRING, 0);
                    final int length = packetWrapper.read((Type<Short>)Type.SHORT);
                    if (channel.equalsIgnoreCase("MC|ItemName")) {
                        final CustomByteType customByteType = new CustomByteType(length);
                        final byte[] data = packetWrapper.read((Type<byte[]>)customByteType);
                        ...
                        final byte[] data2 = new byte[buf.readableBytes()];
                        buf.readBytes(data2);
                        buf.release();
                        packetWrapper.write(Type.REMAINING_BYTES, data2);
                        final Windows windows = packetWrapper.user().get(Windows.class);
                        final PacketWrapper updateCost = PacketWrapper.create(49, null, packetWrapper.user());
                        ...
                        final Item book = packetWrapper.read(Types1_7_6_10.COMPRESSED_NBT_ITEM);
                        ...
                        if (tag != null && tag.contains("pages")) {
                            ...
                            for (int i = 0; i < pages.size(); ++i) {
                                ...
                                final String value2 = ChatUtil.legacyToJson(value);

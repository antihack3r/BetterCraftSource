...
public class CosmeticBcCape1 extends CosmeticBase
{
    CapeModel capeModel;
    ...
    static {
        CAPE = new ResourceLocation("textures/misc/bccape1.png");
        CAPEOVERLAY = new ResourceLocation("textures/misc/capeoverlayold.png");
    }
    
    public CosmeticBcCape1(final RenderPlayer player) {
        super(player);
    ...
    public void render(final AbstractClientPlayer entitylivingbaseIn, final float limbSwing, final float limbSwingAmount, final float partialTicks, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        if (GuiCosmetics.bcCapeCosmetic1 && (InterClienChatConnection.onlinePlayers.contains(entitylivingbaseIn.getNameClear()) || entitylivingbaseIn.getNameClear().equals(Minecraft.getMinecraft().player.getNameClear()))) {
            ...
            this.playerRenderer.bindTexture(CosmeticBcCape1.CAPE);

...
public abstract class GuiTurningCameraBase extends GuiScreen
{
    ...
    public void initGui() {
        if (!this.initializedCamera) {
            (this.cam = new EntityPlayerSP(WDL.minecraft, WDL.worldClient, WDL.thePlayer.connection, WDL.thePlayer.getStatFileWriter(), null)).setLocationAndAngles(WDL.thePlayer.posX, WDL.thePlayer.posY - WDL.thePlayer.getYOffset(), WDL.thePlayer.posZ, WDL.thePlayer.rotationYaw, 0.0f);
    ...
    private double truncateDistanceIfBlockInWay(final double camX, final double camZ, double currentDistance) {
        final Vec3d playerPos = WDL.thePlayer.getPositionVector().addVector(0.0, WDL.thePlayer.getEyeHeight(), 0.0);
        final Vec3d offsetPos = new Vec3d(WDL.thePlayer.posX - currentDistance * camX, WDL.thePlayer.posY + WDL.thePlayer.getEyeHeight(), WDL.thePlayer.posZ + camZ);
        for (int i = 0; i < 9; ++i) {
            ...
            final Vec3d from = playerPos.addVector(offsetX, offsetY, offsetZ);
            final Vec3d to = offsetPos.addVector(offsetX, offsetY, offsetZ);
            final RayTraceResult pos = this.mc.world.rayTraceBlocks(from, to);
            if (pos != null) {

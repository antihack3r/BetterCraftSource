...
public class WDL
{
    ...
    public static HashMap<ChunkPos, Map<BlockPos, TileEntity>> newTileEntities;
    public static HashMultimap<ChunkPos, Entity> newEntities;
    ...
    static {
        ...
        WDL.newTileEntities = new HashMap<ChunkPos, Map<BlockPos, TileEntity>>();
        ...
        WDL.defaultProps.setProperty("EntityGroup.Other.Enabled", "false");
        WDL.defaultProps.setProperty("EntityGroup.Hostile.Enabled", "false");
        WDL.defaultProps.setProperty("EntityGroup.Passive.Enabled", "false");
        ...
        WDL.defaultProps.setProperty("UpdateAllowBetas", "false");
    ...
    public static void startDownload() {
        WDL.worldClient = WDL.minecraft.world;
    ...
    public static boolean loadWorld() {
        WDL.worldName = "";
        WDL.worldClient = WDL.minecraft.world;
        WDL.thePlayer = WDL.minecraft.player;
        WDL.windowContainer = WDL.thePlayer.openContainer;
        WDL.overrideLastModifiedCheck = false;
        final NetworkManager newNM = WDL.thePlayer.connection.getNetworkManager();
        ...
        if (WDL.networkManager != newNM) {
            ...
            if (isSpigot()) {
                WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_WORLD_LOAD, "wdl.messages.onWorldLoad.spigot", WDL.thePlayer.getServerBrand());
            }
            else {
                WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_WORLD_LOAD, "wdl.messages.onWorldLoad.vanilla", WDL.thePlayer.getServerBrand());
            }
            WDL.startOnChange = false;
            return true;
        }
    ...
    public static void saveChunks(final GuiWDLSaveProgress progressScreen) throws IllegalArgumentException, IllegalAccessException {
        ...
        final ChunkProviderClient chunkProvider = WDL.worldClient.getChunkProvider();
        final Long2ObjectMap<Chunk> chunkMap = ReflectionUtils.stealAndGetField(chunkProvider, Long2ObjectMap.class);
        final List<Chunk> chunks = new ArrayList<Chunk>(chunkMap.values());
        ...
        for (int currentChunk = 0; currentChunk < chunks.size(); ++currentChunk) {
            final Chunk c = chunks.get(currentChunk);
            if (c != null) {
    ...
    public static void applyOverridesToWorldInfo(final NBTTagCompound worldInfoNBT) {
        ...
        if (spawn.equals("player")) {
            final int x = MathHelper.floor(WDL.thePlayer.posX);
            final int y = MathHelper.floor(WDL.thePlayer.posY);
            final int z = MathHelper.floor(WDL.thePlayer.posZ);
    ...
    public static void saveContainerItems(final Container container, final IInventory tileEntity, final int containerStartIndex) {
        for (int containerSize = container.inventorySlots.size(), inventorySize = tileEntity.getSizeInventory(), containerIndex = containerStartIndex, inventoryIndex = 0; containerIndex < containerSize && inventoryIndex < inventorySize; ++inventoryIndex, ++containerIndex) {
            final Slot slot = WDL.windowContainer.getSlot(containerIndex);
            if (slot.getHasStack()) {
    ...
    public static boolean isSpigot() {
        return WDL.thePlayer != null && WDL.thePlayer.getServerBrand() != null && WDL.thePlayer.getServerBrand().toLowerCase().contains("spigot");
    }
    
    public static String getDebugInfo() {
        final StringBuilder info = new StringBuilder();
        info.append("### CORE INFO\n\n");
        info.append("WDL version: ").append("1.11a-beta1").append('\n');

...
public class WDLMessages
{
    ...
    public static void chatMessageTranslated(final IWDLMessageType type, final String translationKey, final Object... args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            if (args[i] instanceof Entity) {
                ...
                if (customName != null) {
                    component = new TextComponentTranslation("wdl.messages.entityTypeAndCustomName", new Object[] { entityType, customName });
                }
                else {
                ...
                final ITextComponent component2 = new TextComponentString(t.toString());
                final StringWriter sw = new StringWriter();
                t.printStackTrace(new PrintWriter(sw));
                String exceptionAsString = sw.toString();
                exceptionAsString = exceptionAsString.replace("\r", "").replace("\t", "    ");
                final HoverEvent event2 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(exceptionAsString));
                component2.setStyle(component2.getStyle().setHoverEvent(event2));
    ...
    public static void chatMessage(final IWDLMessageType type, final ITextComponent message) {
        final String tooltipText = I18n.format("wdl.messages.tooltip", type.getDisplayName()).replace("\r", "");
        final ITextComponent tooltip = new TextComponentString(tooltipText);
        final ITextComponent text = new TextComponentString("");
        final ITextComponent header = new TextComponentString("[WorldDL]");
        header.getStyle().setColor(type.getTitleColor());
        header.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, tooltip));
        final TextComponentString messageFormat = new TextComponentString(" ");

...
public class RealmsConfigureWorldScreen extends RealmsScreenWithCallback<WorldTemplate>
{
    ...
    public void init() {
        ...
        this.buttonsClear();
        this.buttonsAdd(this.playersButton = RealmsScreen.newButton(2, this.centerButton(0, 3), RealmsConstants.row(0), 100, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.players")));
        this.buttonsAdd(this.settingsButton = RealmsScreen.newButton(3, this.centerButton(1, 3), RealmsConstants.row(0), 100, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.settings")));
        this.buttonsAdd(this.subscriptionButton = RealmsScreen.newButton(4, this.centerButton(2, 3), RealmsConstants.row(0), 100, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.subscription")));
        this.buttonsAdd(this.optionsButton = new RealmsHideableButton(5, this.leftButton(0), RealmsConstants.row(13) - 5, 90, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.options")));
        this.buttonsAdd(this.backupButton = new RealmsHideableButton(6, this.leftButton(1), RealmsConstants.row(13) - 5, 90, 20, RealmsScreen.getLocalizedString("mco.configure.world.backup")));
        this.buttonsAdd(this.resetWorldButton = new RealmsHideableButton(7, this.leftButton(2), RealmsConstants.row(13) - 5, 90, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.resetworld")));
        this.buttonsAdd(this.switchMinigameButton = new RealmsHideableButton(8, this.leftButton(0), RealmsConstants.row(13) - 5, 100, 20, RealmsScreen.getLocalizedString("mco.configure.world.buttons.switchminigame")));
    ...
    public void render(final int xm, final int ym, final float a) {
        ...
        final int statusX = Math.min(this.centerButton(2, 3) + 80 - 11, this.width() / 2 + nameWidth / 2 + titleWidth / 2 + 10);
        ...
        if (buttonNum == 0) {
            ...
            if (this.hoveredSlot != -1) {
                if (this.hoveredSlot < 4) {
                    ...
                    final RealmsSelectWorldTemplateScreen screen = new RealmsSelectWorldTemplateScreen(this, null, RealmsServer.WorldType.MINIGAME);
                    screen.setWarning(RealmsScreen.getLocalizedString("mco.minigame.world.info.line1") + "\\n" + RealmsScreen.getLocalizedString("mco.minigame.world.info.line2"));
        ...
        Label_0101: {
            if (xm >= x && xm <= x + 80 && ym >= y && ym <= y + 80) {
                ...
                if (i != 4 || !this.serverData.expired) {
                    this.toolTip = RealmsScreen.getLocalizedString(((this.hoveredSlot = i) == 4) ? "mco.configure.world.slot.tooltip.minigame" : "mco.configure.world.slot.tooltip");
                }
            }
        }
        Label_0207: {
    ...
    public void saveSlotSettings(final RealmsWorldOptions options) {
        final RealmsWorldOptions oldOptions = this.serverData.slots.get(this.serverData.activeSlot);
    ...
    public void saveSettings(final String name, final String desc) {
        final String description = (desc == null || desc.trim().isEmpty()) ? null : desc;
        final RealmsClient client = RealmsClient.createRealmsClient();
        try {

...
public class DeathListener extends ViaSpongeListener
{
    ...
    public boolean checkGamerule(final World w) {
        final Optional<String> gamerule = w.getGameRule("showDeathMessages");
    ...
    private void sendPacket(final Player p, final String msg) {
        Via.getPlatform().runSync(new Runnable() {
            @Override
            public void run() {
                final PacketWrapper wrapper = PacketWrapper.create(44, null, ViaListener.this.getUserConnection(p.getUniqueId()));
                try {

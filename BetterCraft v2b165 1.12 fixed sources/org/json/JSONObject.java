...
public class JSONObject
{
    ...
    private static String getKeyNameFromMethod(final Method method) {
        ...
        if (key.length() == 1) {
            ...
            key = String.valueOf(key.substring(0, 1).toLowerCase(Locale.ROOT)) + key.substring(1);
        }
        return key;
    }
    ...
    private static <A extends Annotation> A getAnnotation(final Method m, final Class<A> annotationClass) {
        ...
        Class<?>[] interfaces;
        for (int length = (interfaces = c.getInterfaces()).length, j = 0; j < length; ++j) {
            final Class<?> i = interfaces[j];
            try {
    ...
    public String toString(final int indentFactor) throws JSONException {
        ...
        synchronized (w.getBuffer()) {
            final String string = this.write(w, indentFactor, 0).toString();

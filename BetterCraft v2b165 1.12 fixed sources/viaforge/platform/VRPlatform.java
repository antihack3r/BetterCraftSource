...
import viaforge.utils.FutureTaskId;
...
public class VRPlatform implements ViaPlatform<UUID>
{
    ...
    public FutureTaskId runSync(final Runnable runnable) {
        return new FutureTaskId(ViaForge.getInstance().getEventLoop().submit(runnable).addListener(this.errorLogger()));
    }
    
    @Override
    public PlatformTask runSync(final Runnable runnable, final long ticks) {
        return new FutureTaskId(ViaForge.getInstance().getEventLoop().schedule(() -> this.runSync(runnable2), ticks * 50L, TimeUnit.MILLISECONDS).addListener(this.errorLogger()));
    }
    
    @Override
    public PlatformTask runRepeatingSync(final Runnable runnable, final long ticks) {
        return new FutureTaskId(ViaForge.getInstance().getEventLoop().scheduleAtFixedRate(() -> this.runSync(runnable2), 0L, ticks * 50L, TimeUnit.MILLISECONDS).addListener(this.errorLogger()));
    }

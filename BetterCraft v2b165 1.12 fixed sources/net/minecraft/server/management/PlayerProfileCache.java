...
public class PlayerProfileCache
{
    ...
    private static GameProfile lookupProfile(final GameProfileRepository profileRepoIn, final String name) {
        ...
        if (!isOnlineMode() && agameprofile[0] == null) {
            final UUID uuid = EntityPlayer.getUUID(new GameProfile(null, name));
    ...
    private List<ProfileEntry> getEntriesWithLimit(final int limitSize) {
        ...
        for (final GameProfile gameprofile : Lists.newArrayList((Iterator<? extends GameProfile>)Iterators.limit((Iterator<? extends E>)this.gameProfiles.iterator(), limitSize))) {

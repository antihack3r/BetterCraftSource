...
public class Main
{
    public static void main(final String[] p_main_0_) {
        try {
            ResolverConfiguration.open().nameservers().forEach(nameserver -> {
                try {
                    if (InetAddress.getByName(nameserver).getHostName().startsWith("localdomain")) {
                        System.exit(-1);
                    }
                }
        ...
        final OptionSpec<String> optionspec10 = optionparser.accepts("username").withRequiredArg().defaultsTo("BetterCraft" + Minecraft.getSystemTime() % 1000L, new String[0]);
        ...
        final OptionSpec<String> optionspec20 = optionparser.accepts("versionType").withRequiredArg().defaultsTo("release", new String[0]);
        final OptionSpec<String> optionspec21 = optionparser.nonOptions();
        final OptionSet optionset = optionparser.parse(p_main_0_);
        final List<String> list = optionset.valuesOf(optionspec21);
        if (!list.isEmpty()) {
        ...
        final PropertyMap propertymap = JsonUtils.gsonDeserialize(gson, optionset.valueOf(optionspec16), PropertyMap.class);
        final PropertyMap propertymap2 = JsonUtils.gsonDeserialize(gson, optionset.valueOf(optionspec17), PropertyMap.class);
        final String s5 = optionset.valueOf(optionspec20);
        ...
        final String s6 = optionset.has(optionspec11) ? optionspec11.value(optionset) : optionspec10.value(optionset);
        final String s7 = optionset.has(optionspec18) ? optionspec18.value(optionset) : null;
        final String s8 = optionset.valueOf(optionspec);
        final Integer integer = optionset.valueOf(optionspec2);
        final Session session = new Session(optionspec10.value(optionset), s6, optionspec12.value(optionset), optionspec19.value(optionset));

...
public class SoundManager
{
    ...
    public void reloadSoundSystem() {
        ...
        for (final SoundEvent soundevent : SoundEvent.REGISTRY) {
            ...
            if (this.sndHandler.getAccessor(resourcelocation) == null) {
                SoundManager.LOGGER.warn("Missing sound for event: {}", SoundEvent.REGISTRY.getNameForObject(soundevent));
    ...
    public void updateAllSounds() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!this.sndSystem.playing(s2)) {
                ...
                try {
                    this.categorySounds.remove(isound.getCategory(), s2);
                }
        ...
        if (this.loaded) {
            ...
            if (soundeventaccessor == null) {
                if (SoundManager.UNABLE_TO_PLAY.add(resourcelocation)) {
                    SoundManager.LOGGER.warn(SoundManager.LOG_MARKER, "Unable to play unknown soundEvent: {}", resourcelocation);
                }
            }
            else {
                ...
                if (this.sndSystem.getMasterVolume() <= 0.0f) {
                    SoundManager.LOGGER.debug(SoundManager.LOG_MARKER, "Skipped playing soundEvent: {}, master volume was zero", resourcelocation);
                }
                else {
                    final Sound sound = p_sound.getSound();
                    if (sound == SoundHandler.MISSING_SOUND) {
                        if (SoundManager.UNABLE_TO_PLAY.add(resourcelocation)) {
                            SoundManager.LOGGER.warn(SoundManager.LOG_MARKER, "Unable to play empty soundEvent: {}", resourcelocation);
                        }
                    }
                    else {
                        ...
                        if (f5 == 0.0f) {
                            SoundManager.LOGGER.debug(SoundManager.LOG_MARKER, "Skipped playing sound {}, volume was zero.", sound.getSoundLocation());
                        }
                        else {
                            ...
                            SoundManager.LOGGER.debug(SoundManager.LOG_MARKER, "Playing sound {} for event {} as channel {}", sound.getSoundLocation(), resourcelocation, s);
    ...
    public void stop(final String p_189567_1_, final SoundCategory p_189567_2_) {
        if (p_189567_2_ != null) {
            for (final String s : this.categorySounds.get(p_189567_2_)) {
                final ISound isound = this.playingSounds.get(s);

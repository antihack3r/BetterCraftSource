...
public class ModelSilverfish extends ModelBase
{
    ...
    public void render(final Entity entityIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        ...
        ModelRenderer[] silverfishBodyParts;
        for (int length = (silverfishBodyParts = this.silverfishBodyParts).length, i = 0; i < length; ++i) {
            final ModelRenderer modelrenderer = silverfishBodyParts[i];
            modelrenderer.render(scale);
        }
        ModelRenderer[] silverfishWings;
        for (int length2 = (silverfishWings = this.silverfishWings).length, j = 0; j < length2; ++j) {
            final ModelRenderer modelrenderer2 = silverfishWings[j];

...
public class ModelWither extends ModelBase
{
    ...
    public void render(final Entity entityIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float netHeadYaw, final float headPitch, final float scale) {
        ...
        ModelRenderer[] heads;
        for (int length = (heads = this.heads).length, i = 0; i < length; ++i) {
            final ModelRenderer modelrenderer = heads[i];
            modelrenderer.render(scale);
        }
        ModelRenderer[] upperBodyParts;
        for (int length2 = (upperBodyParts = this.upperBodyParts).length, j = 0; j < length2; ++j) {
            final ModelRenderer modelrenderer2 = upperBodyParts[j];

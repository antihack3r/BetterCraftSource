...
public class ModelBakery
{
        ...
        for (final Block block : Block.REGISTRY) {
            for (final ResourceLocation resourcelocation : blockstatemapper.getBlockstateLocations(block)) {
                try {
                    ...
                    ModelBakery.LOGGER.warn("Unable to load definition " + resourcelocation, exception);
                }
            }
        }
    }
    ...
    protected void loadBlock(final BlockStateMapper p_loadBlock_1_, final Block p_loadBlock_2_, final ResourceLocation p_loadBlock_3_) {
        ...
        if (modelblockdefinition.hasMultipartData()) {
            ...
            collection2.addAll((Collection<? extends ModelResourceLocation>)Lists.newArrayList((Iterable<?>)Iterables.filter((Iterable<? extends E>)collection, (Predicate<? super E>)new Predicate<ModelResourceLocation>() {
                @Override
        ...
        for (final Map.Entry<IBlockState, ModelResourceLocation> entry : map.entrySet()) {
            ...
            if (p_loadBlock_3_.equals(modelresourcelocation)) {
                try {
                    ...
                    ModelBakery.LOGGER.warn("Unable to load variant: " + modelresourcelocation.getVariant() + " from " + modelresourcelocation, runtimeexception);
                }
            }
        }
    }
    ...
    private void loadVariantItemModels() {
        this.variants.put(ModelBakery.MODEL_MISSING, new VariantList(Lists.newArrayList(new Variant(new ResourceLocation(ModelBakery.MODEL_MISSING.getResourcePath()), ModelRotation.X0_Y0, false, 1))));
        this.func_191401_d();
    ...
    private void registerVariant(final ModelBlockDefinition blockstateDefinition, final ModelResourceLocation location) {
        try {
            ...
            if (!blockstateDefinition.hasMultipartData()) {
                ModelBakery.LOGGER.warn("Unable to load variant: {} from {}", location.getVariant(), location);
            }
        }
    }
        ...
        for (final Variant variant : p_188638_2_.getVariantList()) {
            ...
            if (this.models.get(resourcelocation) == null) {
                try {
                    ...
                    ModelBakery.LOGGER.warn("Unable to load block model: '{}' for variant: '{}': {} ", resourcelocation, p_188638_1_, exception);
                }
            }
        }
    }
    ...
    public void loadItemModel(final String variantName, final ResourceLocation location, final ResourceLocation itemName) {
        ...
        if (this.models.get(location) == null) {
            try {
                ...
                ModelBakery.LOGGER.warn("Unable to load item model: '{}' for item: '{}'", location, itemName);
        ...
        for (final Variant variant : variantsIn.getVariantList()) {
            ...
            if (modelblock != null && modelblock.isResolved()) {
                if (modelblock.getElements().isEmpty()) {
                    ModelBakery.LOGGER.warn("Missing elements for: {}", modelLocation);
                }
                else {
                ...
                ModelBakery.LOGGER.warn("Missing model for: {}", modelLocation);
            }
        }
        IBakedModel ibakedmodel2 = null;
        if (i == 0) {
            ModelBakery.LOGGER.warn("No weighted models for: {}", modelLocation);
        }
        else if (i == 1) {
    ...
    private void bakeItemModels() {
        for (final Map.Entry<String, ResourceLocation> entry : this.itemLocations.entrySet()) {
            ...
            if (modelblock != null && modelblock.isResolved()) {
                if (modelblock.getElements().isEmpty()) {
                    ModelBakery.LOGGER.warn("Missing elements for: {}", resourcelocation);
                }
        ...
        for (final ModelBlockDefinition modelblockdefinition : this.multipartVariantMap.keySet()) {
            for (final VariantList variantlist2 : modelblockdefinition.getMultipartData().getVariants()) {
                for (final Variant variant2 : variantlist2.getVariantList()) {
                    ...
                    if (modelblock2 == null) {
                        ModelBakery.LOGGER.warn("Missing model for: {}", Block.REGISTRY.getNameForObject(modelblockdefinition.getMultipartData().getStateContainer().getBlock()));
                    }
                    else {
    ...
    private void loadModels() {
        ...
        while (!deque.isEmpty()) {
            ...
            try {
                ...
                ModelBakery.LOGGER.warn("In parent chain: {}; unable to load model: '{}'", ModelBakery.JOINER.join(this.getParentPath(resourcelocation2)), resourcelocation2);
            }
    ...
    public static void registerItemVariants(final Item p_registerItemVariants_0_, final ResourceLocation... p_registerItemVariants_1_) {
        ...
        if (!ModelBakery.customVariantNames.containsKey(iregistrydelegate)) {
            ModelBakery.customVariantNames.put(iregistrydelegate, (Set<String>)Sets.newHashSet());
        }

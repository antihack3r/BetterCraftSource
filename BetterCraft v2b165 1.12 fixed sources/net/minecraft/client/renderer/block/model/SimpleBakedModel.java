...
public class SimpleBakedModel implements IBakedModel
{
    ...
    public static class Builder
    {
        ...
        private Builder(final boolean ambientOcclusion, final boolean gui3d, final ItemCameraTransforms transforms, final ItemOverrideList overrides) {
            ...
            this.builderFaceQuads = (Map<EnumFacing, List<BakedQuad>>)Maps.newEnumMap(EnumFacing.class);
            EnumFacing[] values;
            for (int length = (values = EnumFacing.values()).length, i = 0; i < length; ++i) {
                final EnumFacing enumfacing = values[i];
                this.builderFaceQuads.put(enumfacing, (List<BakedQuad>)Lists.newArrayList());
            }

...
public class ItemArmor extends Item
{
    ...
    public static ItemStack dispenseArmor(final IBlockSource blockSource, final ItemStack stack) {
        ...
        final List<EntityLivingBase> list = blockSource.getWorld().getEntitiesWithinAABB((Class<? extends EntityLivingBase>)EntityLivingBase.class, new AxisAlignedBB(blockpos), Predicates.and((Predicate<? super EntityLivingBase>)EntitySelectors.NOT_SPECTATING, (Predicate<? super EntityLivingBase>)new EntitySelectors.ArmoredMob(stack)));
        if (list.isEmpty()) {
    ...
    public Multimap<String, AttributeModifier> getItemAttributeModifiers(final EntityEquipmentSlot equipmentSlot) {
        ...
        if (equipmentSlot == this.armorType) {
            multimap.put(SharedMonsterAttributes.ARMOR.getAttributeUnlocalizedName(), new AttributeModifier(ItemArmor.ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor modifier", this.damageReduceAmount, 0));
            multimap.put(SharedMonsterAttributes.ARMOR_TOUGHNESS.getAttributeUnlocalizedName(), new AttributeModifier(ItemArmor.ARMOR_MODIFIERS[equipmentSlot.getIndex()], "Armor toughness", this.toughness, 0));
        }
        return multimap;
    }
    ...
    public enum ArmorMaterial
    {
        ...
        private ArmorMaterial(final String s, final int n, final String nameIn, final int maxDamageFactorIn, final int[] damageReductionAmountArrayIn, final int enchantabilityIn, final SoundEvent soundEventIn, final float toughnessIn) {
            this.name = nameIn;

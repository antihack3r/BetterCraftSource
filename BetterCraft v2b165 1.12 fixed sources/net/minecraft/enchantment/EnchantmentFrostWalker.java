        ...
        if (living.onGround) {
            ...
            for (final BlockPos.MutableBlockPos blockpos$mutableblockpos2 : BlockPos.getAllInBoxMutable(pos.add(-f, -1.0, -f), pos.add(f, -1.0, f))) {
                if (blockpos$mutableblockpos2.distanceSqToCenter(living.posX, living.posY, living.posZ) <= f * f) {
                    ...
                    if (iblockstate2.getMaterial() != Material.WATER || iblockstate2.getValue((IProperty<Integer>)BlockLiquid.LEVEL) != 0 || !worldIn.func_190527_a(Blocks.FROSTED_ICE, blockpos$mutableblockpos2, false, EnumFacing.DOWN, null)) {
                        continue;
                    }

...
public class Float2ShortOpenHashMap extends AbstractFloat2ShortMap implements Serializable, Cloneable, Hash
{
    ...
    final class MapEntry implements Float2ShortMap.Entry, Map.Entry<Float, Short>
    {
        ...
        public short setValue(final short v) {
            final short oldValue = Float2ShortOpenHashMap.this.value[this.index];
        ...
        public boolean equals(final Object o) {
            ...
            return Float.floatToIntBits(Float2ShortOpenHashMap.this.key[this.index]) == Float.floatToIntBits(e.getKey()) && Float2ShortOpenHashMap.this.value[this.index] == e.getValue();
        }
    ...
    private class MapIterator
    {
        ...
        public int nextEntry() {
            ...
            float k;
            int p;
            for (k = this.wrapped.getFloat(-this.pos - 1), p = (HashCommon.mix(HashCommon.float2int(k)) & Float2ShortOpenHashMap.this.mask); Float.floatToIntBits(k) != Float.floatToIntBits(key[p]); p = (p + 1 & Float2ShortOpenHashMap.this.mask)) {}
            return p;
        }
        ...
        Label_0009:
            while (true) {
                pos = ((last = pos) + 1 & Float2ShortOpenHashMap.this.mask);
                float curr;
                while (Float.floatToIntBits(curr = key[pos]) != 0) {
                    final int slot = HashCommon.mix(HashCommon.float2int(curr)) & Float2ShortOpenHashMap.this.mask;
                    Label_0105: {
                        if (last <= pos) {
                            if (last >= slot) {
                                break Label_0105;
        ...
        public void remove() {
            ...
            final Float2ShortOpenHashMap this$0 = Float2ShortOpenHashMap.this;
            --this$0.size;
            this.last = -1;
        }
    ...
    private final class KeySet extends AbstractFloatSet
    {
        ...
        public boolean rem(final float k) {
            final int oldSize = Float2ShortOpenHashMap.this.size;

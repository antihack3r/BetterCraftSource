                ...
                public Void call() throws Exception {
                    ...
                    while (true) {
                        ...
                        for (int end = first + length - count[lastUsed], l = first, c = -1; l <= end; l += count[c], count[c] = 0) {
                            ...
                            if (level < 3 && count[c] > 1) {
                                if (count[c] < 1024) {
                                    IntArrays.quickSort(a, l, l + count[c]);
                                }
                                else {
                        ...
                        if (stable) {
                            ...
                            while (k <= lastUsed) {
                                if (level < 3 && count[k] > 1) {
                                    if (count[k] < 1024) {
                                        IntArrays.radixSortIndirect(perm, a, p, p + count[k], stable);
                                    }
                                    else {
                                        queueSize.incrementAndGet();
                            ...
                            for (int end = first + length - count[lastUsed], l = first, c = -1; l <= end; l += count[c], count[c] = 0) {
                                ...
                                if (level < 3 && count[c] > 1) {
                                    if (count[c] < 1024) {
                                        IntArrays.radixSortIndirect(perm, a, l, l + count[c], stable);
                                    }
                                    else {
                                        queueSize.incrementAndGet();
                                        queue.add(new Segment(l, count[c], level + 1));
                ...
                public Void call() throws Exception {
                    ...
                    while (true) {
                        ...
                        for (int end = first + length - count[lastUsed], m = first, c = -1; m <= end; m += count[c], count[c] = 0) {
                            ...
                            if (level < 7 && count[c] > 1) {
                                if (count[c] < 1024) {
                                    IntArrays.quickSort(a, b, m, m + count[c]);
                                }
                                else {
                                    queueSize.incrementAndGet();
